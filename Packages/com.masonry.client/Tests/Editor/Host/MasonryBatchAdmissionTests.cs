...
{
    public sealed class MasonryBatchAdmissionTests
    {
        [Test]
        public void SerializedStartModesAreAdmittedAndOldDuplicatesAreIgnored()
        {
            ...
            MasonryLogRecord[] admitted = harness
                .Logger.Records.Where(record => record.EventName == "masonry.batch.admitted")
                .ToArray();
            ...
            Assert.That(
                harness.Logger.Records.Count(record =>
                    record.EventName == "masonry.batch.duplicate"
                ),
                Is.EqualTo(1)
            );
        ...
        public void BoundarySizedSerializedBatchesAreAccepted()
        {
            ...
            Assert.That(
                harness.Logger.Records.Count(record =>
                    record.EventName == "masonry.batch.admitted"
                ),
                Is.EqualTo(2)
            );
            Assert.That(harness.Transport.SubmitMessages, Has.Count.EqualTo(1));
        ...
        public void BatchValidationFailuresAreSubmittedWithAvailableIdentity()
        {
            ...
            foreach ((Batch batch, CoreErrorCode code, CommandId? commandId) in cases)
            {
                harness.Transport.EnqueueSubmit(
                    FakeMasonryTransport.ResponseResult(BatchResponse(session, batch))
                );
                harness.Transport.EnqueueSubmit(
        ...
        {
            using MasonryTestHarness oversizedHarness = MasonryTestHarness.Create();
            SessionId session = new(Guid.NewGuid());
            ...
            using MasonryTestHarness malformedHarness = MasonryTestHarness.Create();
            Connect(malformedHarness, session);
            byte[] malformed = MasonryMessagePack
                .SerializeResponse(BatchResponse(session, ValidBatch(session, BatchStart.Now)))
                .Take(12)
                .ToArray();
            malformedHarness.Transport.EnqueueSubmit(
                new MasonryTransportResult(MasonryTransportStatus.Success, malformed)
            );
        ...
        private static void Connect(MasonryTestHarness harness, SessionId session)
        {
            harness.Transport.EnqueueConnect(FakeMasonryTransport.SnapshotResponse(session));
            harness.Runner.Connect();
        }

        private static void SubmitResponse(MasonryTestHarness harness, Response response)
        {
