...
{
    public sealed class MasonryFailureSubmissionTests
    {
        [Test]
        public void BatchAndOperationFailuresPreserveIdsCodesAndBoundedDiagnostics()
        {
            ...
            MasonryLogRecord batchLog = harness.Logger.Records.Single(record =>
                record.EventName == "masonry.batch.failed"
            );
            ...
            Assert.That(
                harness
                    .Logger.Records.Single(record => record.EventName == "masonry.operation.failed")
                    .Fields!["error_code"],
        ...
        public void EveryCoreErrorCodeUsesItsExactProtocolName()
        {
            ...
            {
                byte[] bytes = MasonryMessagePack.SerializeBatchFailure(
        ...
        public void ReturnedCorrectionAppliesAfterTheCurrentResponseWithoutRecursion()
        {
            ...
            harness.Transport.EnqueueSubmit(
                new MasonryTransportResult(MasonryTransportStatus.Success, new byte[] { 2 })
            );
            harness.Transport.EnqueueSubmit(
                new MasonryTransportResult(MasonryTransportStatus.Success, new byte[] { 3 })
            );
        ...
        public void FailureSubmissionTransportErrorIsSessionFatal()
        {
            ...
            Assert.That(
                ...
                Is.EqualTo(new[] { "masonry.operation.failed", "masonry.session.failed" })
            );
        }
        ...
        private static MasonryTransportResult SnapshotResponse(
            SessionId session,
            bool inputDisabled
        ) => FakeMasonryTransport.ResponseResult(Response(session, inputDisabled));
        ...
        private sealed class ReentrantFailureCodec : IMasonryProtocolCodec
        {
