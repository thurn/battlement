...
{
    public sealed class MasonryCustomCommandTests
    {
        ...
        public void UnregisteredAndMalformedPayloadsFailOnlyTheirBatches()
        {
            ...
            Assert.That(
                harness
                    .Logger.Records.Last(record => record.EventName == "masonry.batch.failed")
                    .Fields!["error_code"],
                Is.EqualTo(nameof(CoreErrorCode.HandlerNotRegistered))
            );

            using MasonryTestHarness malformed = MasonryTestHarness.Create();
        ...
        public void TrackedWorkBlocksFailsLateAndReceivesCancellation()
        {
            ...
            harness.Transport.EnqueuePoll(FakeMasonryTransport.SnapshotResponse(session));
            harness.Runner.RunFrame();
        ...
        private static void Register(MasonryTestHarness harness, FixtureHandler handler) =>
        ...
        )
        {
            ...
            harness.Transport.EnqueueConnect(
                FakeMasonryTransport.SnapshotResponse(session, objects: new[] { target })
            );
        ...
        private static MasonryTransportResult Result(Response<ICommand> response) =>
            new(
                MasonryTransportStatus.Success,
                MasonryMessagePack.SerializeResponse(response, PayloadFormatter)
            );

        private static MasonryTransportResult EmptyResult(SessionId session) =>
        ...
        private static ClientMessage<FixtureError, FlashPayload> DecodeCustom(byte[] message) =>
            MasonryMessagePack.DeserializeClientMessage(message, ErrorFormatter, PayloadFormatter);
    }
}
