...
namespace Masonry
{
    internal sealed class MasonryParticleEffects : IDisposable
    {
        ...
        public MasonryParticleEffects(MasonryWorld world, MasonryPreparedAssets preparedAssets)
        {
            this.world = world;
        ...
        public IMasonryCommandOperation? Play(CommandBody.Particle.Play command)
        {
        ...
        public IMasonryCommandOperation? Stop(CommandBody.Particle.Stop command)
        {
        ...
        public IMasonryCommandOperation Spawn(
            CommandId commandId,
        ...
        )
        {
            ...
            try
            {
                ...
                if (!prefab.TryGetComponent(out MasonryEffectPool marker))
                {
        ...
        private static void RequirePoolLimit(int value)
        {
            if (value is < 1 or > 128)
            {
                throw new MasonryCommandException(
                    CoreErrorCode.InvalidProperty,
                    "MasonryEffectPool maxInactiveCount must be between 1 and 128."
                );
            }
        }
        ...
        private sealed class EffectPool
        {
            ...
            private IMasonryAssetLease? creationLease;
            ...
            public EffectInstance Get(IMasonryAssetLease lease)
            {
                if (isRetired)
                {
        ...
        private sealed class EffectInstance
        {
            ...
            private void InvokeResets(bool acquiring)
            {
                ...
                {
                    if (component is not IMasonryPoolReset reset)
                    {
                        continue;
                    }
                    ...
                    if (acquiring)
                    {
                        reset.OnMasonryAcquire();
                    }
                    else
                    {
                        reset.OnMasonryRelease();
                    }
                }
            }
        ...
        private sealed class EffectOperation : IMasonryCommandOperation
        {
