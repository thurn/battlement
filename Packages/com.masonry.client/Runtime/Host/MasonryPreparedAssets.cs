...
namespace Masonry
{
    ...
    internal sealed class MasonryPreparedAssets : IDisposable, IMasonryPreparedAssetLookup
    {
        ...
        public MasonryPreparedAssets(IMasonryAssetStorage storage) => this.storage = storage;
        ...
        /// </summary>
        public IMasonryAssetLease Acquire(PreparedAsset asset)
        {
            ThrowIfDisposed();
            if (
        ...
        private void ThrowIfDisposed()
        {
            if (isDisposed)
            {
                throw new ObjectDisposedException(nameof(MasonryPreparedAssets));
            }
        }

        private sealed class Entry : IDisposable
        {
            public Entry(PreparedAsset asset, IMasonryAssetHandle handle)
            {
                Asset = asset;
                Handle = handle;
            }
            ...
            public IMasonryAssetHandle Handle { get; }
        ...
        private sealed class Lease : IMasonryAssetLease
        {
            private readonly MasonryPreparedAssets owner;
            private Entry? entry;

            public Lease(MasonryPreparedAssets owner, Entry entry, object value)
            {
                this.owner = owner;
                this.entry = entry;
                Value = value;
