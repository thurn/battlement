...
namespace Google.FlatBuffers
{
    ...
    public class Options
    {
        ...
        public Options(int maxDepth, int maxTables, bool stringEndCheck, bool alignmentCheck)
            : this(maxDepth, maxTables, DEFAULT_MAX_APPARENT_SIZE, stringEndCheck, alignmentCheck)
        { }

        public Options(
            int maxDepth,
            int maxTables,
            ulong maxApparentSize,
            bool stringEndCheck,
            bool alignmentCheck
        )
        {
            ...
            max_apparent_size = maxApparentSize;
        ...
        /// <summary> Maximum expanded traversal bytes, charging repeated references. </summary>
        public ulong maxApparentSize
        {
            get { return max_apparent_size; }
            set { max_apparent_size = value; }
        }
    ...
    public class Verifier
    {
        ...
        private ulong apparent_size_cnt = 0;
        ...
        /// <summary> Bytes visited so far, including repeated references. </summary>
        public ulong apparentSize
        {
            get { return apparent_size_cnt; }
            set { apparent_size_cnt = value; }
        }
        ...
        private bool BufferHasIdentifier(ByteBuffer buf, uint startPos, string identifier)
        {
            if (identifier.Length != FILE_IDENTIFIER_LENGTH)
            {
                throw new ArgumentException(
                    "FlatBuffers: file identifier must be length"
                        + Convert.ToString(FILE_IDENTIFIER_LENGTH)
                );
            }
            for (int i = 0; i < FILE_IDENTIFIER_LENGTH; i++)
            {
                if (
                    (sbyte)identifier[i]
                    != verifier_buffer.GetSbyte(Convert.ToInt32(SIZE_S_OFFSET + i + startPos))
                )
                {
                    return false;
                }
        ...
        private short GetVRelOffset(int pos, short vtableOffset)
        {
            ...
            try
            {
                ...
                int vtable = checked(pos - ReadSOffsetT(verifier_buffer, pos));
        ...
        {
            ulong length = Convert.ToUInt64(verifier_buffer.Length);
            if (elementSize > length || Convert.ToUInt64(pos) > length - elementSize)
            {
                return false;
            }
            if (
                elementSize
                > options.maxApparentSize - Math.Min(apparentSize, options.maxApparentSize)
            )
            {
                return false;
            }
            apparentSize += elementSize;
            return true;
        }
        ...
        {
            var result = new checkElementStruct { elementValid = false, elementOffset = 0 };
        ...
        private bool CheckString(uint pos)
        {
            ...
            if (options.stringEndCheck)
            {
                ...
                result.elementValid =
                    result.elementValid
                    && (verifier_buffer.GetSbyte(Convert.ToInt32(result.elementOffset)) == 0); // Terminating byte must be 0.
            }
        ...
        private bool CheckBufferFromStart(
            string identifier,
            uint startPos,
            VerifyTableAction verifyAction
        )
        {
            if (
                (identifier != null)
                && (identifier.Length != 0)
                && (
                    (verifier_buffer.Length < (SIZE_U_OFFSET + FILE_IDENTIFIER_LENGTH))
                    || (!BufferHasIdentifier(verifier_buffer, startPos, identifier))
                )
            )
            {
                return false;
        ...
        public bool VerifyTableStart(uint tablePos)
        {
            ...
            uint vtable = (uint)(
                tablePos - ReadSOffsetT(verifier_buffer, Convert.ToInt32(tablePos))
            );
            return (
                (CheckComplexity())
                ...
                && (
                    CheckAlignment(
                        Convert.ToUInt32(ReadVOffsetT(verifier_buffer, Convert.ToInt32(vtable))),
                        SIZE_V_OFFSET
                    )
                )
                && (
                    CheckElement(
                        vtable,
                        Convert.ToUInt64(ReadVOffsetT(verifier_buffer, Convert.ToInt32(vtable)))
                    )
                )
            );
        }
        ...
        /// <param name="required"> Required Value when the offset == 0  </param>
        // (this method is used internally by generated verification functions)
        public bool VerifyNestedBuffer(
            uint tablePos,
            short offsetId,
            VerifyTableAction verifyAction,
            bool required
        )
        {
            ...
            if (verifyAction != null)
            {
                ...
                var nestedByteBuffer = new ByteBuffer(
                    verifier_buffer.ToArray(Convert.ToInt32(vecStart), Convert.ToInt32(vecLength))
                );
        ...
        /// <param name="required"> Required Value when the offset == 0 </param>
        // (this method is used internally by generated verification functions)
        public bool VerifyUnion(
            uint tablePos,
            short typeIdVOffset,
            short valueVOffset,
            VerifyUnionAction verifyAction,
            bool required
        )
        {
        ...
        // (this method is used internally by generated verification functions)
        public bool VerifyVectorOfUnion(
            uint tablePos,
            short typeOffsetId,
            short offsetId,
            VerifyUnionAction verifyAction,
        ...
        )
        {
            ...
            if (
                !CheckVector(typeIdVectorOffset, SIZE_BYTE)
        ...
        )
        {
            ...
            {
                // Verification is a trust boundary. Malformed offsets must be reported as
                // invalid input rather than escaping as arithmetic or buffer exceptions.
                return false;
            }
        }
    }
