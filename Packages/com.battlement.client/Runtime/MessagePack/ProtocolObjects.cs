...
namespace Battlement
{
    internal static partial class ProtocolFormat
    {
        ...
        private static void WriteScene(ref MessagePackWriter writer, BattlementScene value)
        {
        ...
        private static BattlementScene ReadScene(ref MessagePackReader reader)
        {
            ReadArrayHeader(ref reader, 2);
            return new BattlementScene(ReadSceneId(ref reader), ReadSceneAddress(ref reader));
        }

        private static void WriteScenes(
            ref MessagePackWriter writer,
            IReadOnlyList<BattlementScene> values
        )
        {
            writer.WriteArrayHeader(values.Count);
            foreach (BattlementScene value in values)
            {
                WriteScene(ref writer, value);
            }
        }

        private static IReadOnlyList<BattlementScene> ReadScenes(ref MessagePackReader reader)
        {
            int count = reader.ReadArrayHeader();
            var values = new BattlementScene[count];
            for (int index = 0; index < count; index++)
            {
        ...
        private static BattlementGameObject ReadGameObject(ref MessagePackReader reader)
        {
            ...
            return new BattlementGameObject(
                id,
                kind,
                parentScene,
                parentId,
        ...
        private static void WriteGameObjects(
            ref MessagePackWriter writer,
            IReadOnlyList<BattlementGameObject> values
        )
        {
            writer.WriteArrayHeader(values.Count);
            foreach (BattlementGameObject value in values)
            {
                WriteGameObject(ref writer, value);
            }
        }

        private static IReadOnlyList<BattlementGameObject> ReadGameObjects(
            ref MessagePackReader reader
        )
        {
            int count = reader.ReadArrayHeader();
            var values = new BattlementGameObject[count];
            for (int index = 0; index < count; index++)
            {
