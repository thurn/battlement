...
{
    /// <summary>Encodes and decodes Battlement protocol values as MessagePack.</summary>
    public sealed class BattlementMessagePack : IBattlementExtensionProtocolCodec
    {
        internal static readonly MessagePackSerializerOptions SerializerOptions =
            MessagePackSerializerOptions.Standard.WithSecurity(MessagePackSecurity.UntrustedData);

        public static BattlementMessagePack Instance { get; } = new();

        private BattlementMessagePack() { }
        ...
        private static T Deserialize<T>(ReadOnlyMemory<byte> bytes, ReaderFunc<T> read)
        {
            try
            {
                ...
                if (!reader.End)
                {
                    throw new MessagePackSerializationException(
                        "A Battlement buffer must contain exactly one MessagePack value."
                    );
                }
            ...
            {
                throw new MessagePackSerializationException(
                    "The buffer is not a valid Battlement MessagePack value.",
                    exception
                );
            }
        }
