...
namespace Battlement.Tests
{
    ...
    {
        [UnityTest]
        public IEnumerator KeyboardTargetsFocusedDocumentAndSynthesizesNavigation()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            var lowerDocument = new ObjectId(Guid.NewGuid());
            var lowerRoot = new ObjectId(Guid.NewGuid());
            var focused = new ObjectId(Guid.NewGuid());
            ...
            ) with
            {
                Ui = new[]
                {
                    new UiDocument(
                        lowerDocument,
                        lowerRoot,
                        Children: new[] { new UiNode(focused, new UiElement.Button()) }
                    ),
                    new UiDocument(upperDocument, upperRoot),
                },
            };
            ...
            yield return null;
            yield return null;
            VisualElement focusedElement = Element(harness, focused);
            var keys = 0;
            var moves = 0;
            focusedElement.RegisterCallback<KeyDownEvent>(_ => keys++);
            focusedElement.RegisterCallback<NavigationMoveEvent>(_ => moves++);
            ...
            using var input = new DittoVirtualInput(
                DittoPlatform.Macos,
                checked((uint)Screen.width),
                checked((uint)Screen.height)
            );
            input.Key("RightArrow", DittoKeyAction.Down);
            harness.Runner.BeginDittoInput();
            bool dispatched = targets.DispatchKey(
            ...
            Assert.That(dispatched, Is.True, failure);
