...
{
    public sealed class BattlementSceneOwnershipTests
    {
        [Test]
        public void SnapshotLoadsAdditiveScenesAndSelectsTheDeclaredPrimary()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            BattlementScene first = ContentScene("game/forest");
            BattlementScene second = ContentScene("game/castle");
            ...
            Assert.That(
                harness.Logger.Records.Where(record =>
            ...
            BattlementIdentity sceneObject = Identity(sceneObjectId);
            BattlementIdentity persistentObject = Identity(persistentObjectId);
            Assert.That(sceneObject.gameObject.scene, Is.EqualTo(SceneManager.GetActiveScene()));
            Assert.That(sceneObject.transform.parent.name, Is.EqualTo("Battlement Scene"));
            Assert.That(persistentObject.gameObject.scene, Is.EqualTo(harness.Scene));
            Assert.That(
                persistentObject.transform.parent.name,
                Is.EqualTo("Battlement Persistent")
            );
            Assert.That(
                harness.Scene.GetRootGameObjects().Select(value => value.name),
                Does.Contain("Battlement host")
            );
        }
        ...
        public void ReplacementReusesExactScenesAndUnloadsOnlyRemovedContent()
        {
            ...
            BattlementScene retained = ContentScene("game/retained");
            BattlementScene removed = ContentScene("game/removed");
        ...
        public void ChangedAddressForTheSameSceneUuidReloadsTheScene()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            SessionId session = new(Guid.NewGuid());
            BattlementScene original = ContentScene("game/original");
            BattlementScene replacement = new(original.Id, new SceneAddress("game/replacement"));
            harness.Transport.EnqueueConnect(
                Response(
                    session,
        ...
        public void ChangedUuidAtTheSameAddressWaitsForUnloadBeforeReloading()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            SessionId session = new(Guid.NewGuid());
            BattlementScene original = ContentScene("game/shared");
            BattlementScene replacement = new(new SceneId(Guid.NewGuid()), original.Address);
        ...
        public void PrimaryCutoverReusesScenesAndLeavesBootstrapContentAlone()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            SessionId session = new(Guid.NewGuid());
            BattlementScene first = ContentScene("game/day");
            BattlementScene second = ContentScene("game/night");
        ...
        public void RemovedPreparedSceneStaysLeasedUntilUnloadCompletes()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            SessionId session = new(Guid.NewGuid());
            BattlementScene scene = ContentScene("game/slow-unload");
            harness.AssetStorage.EnqueueSceneUnloadPending();
            harness.Transport.EnqueueConnect(
                Response(
                    session,
            ...
            BattlementScene replacement = ContentScene("game/replacement-after-slow-unload");
            harness.Transport.EnqueueSubmit(
                Response(
                    session,
                    SceneAssets(replacement),
        ...
        public void ReplacementGatesInputUntilDelayedSceneLoadCompletes()
        {
            using BattlementTestHarness harness = BattlementTestHarness.Create();
            SessionId session = new(Guid.NewGuid());
            BattlementScene retained = ContentScene("game/retained-during-load");
            BattlementScene added = ContentScene("game/delayed-load");
            harness.Transport.EnqueueConnect(
                Response(
                    session,
                    SceneAssets(retained),
        ...
        public void FailedSceneLoadReleasesNewAndRetainedSceneHandles()
        {
            ...
            SessionId session = new(Guid.NewGuid());
            BattlementScene retained = ContentScene("game/retained-before-failure");
            BattlementScene failed = ContentScene("game/failed-load");
            try
            {
                harness.Transport.EnqueueConnect(
                    Response(
        ...
        public void InvalidSceneSetsFailBeforeLoadingAnything()
        {
            ...
            BattlementScene first = ContentScene("game/duplicate");
            BattlementScene duplicate = new(new SceneId(Guid.NewGuid()), first.Address);
            ...
            BattlementScene[] tooMany = Enumerable
                .Range(0, 33)
        ...
        private static PreparedAsset[] SceneAssets(params BattlementScene[] scenes) =>
            scenes.Select(SceneAsset).ToArray();

        private static PreparedAsset SceneAsset(BattlementScene scene) =>
            new PreparedAsset.Scene(scene.Address);

        private static BattlementGameObject EmptyObject(ObjectId id, ParentScene parentScene) =>
            new(
                id,
        ...
        private static FakeSceneHandle HandleFor(
            BattlementTestHarness harness,
            BattlementScene scene
        ) =>
