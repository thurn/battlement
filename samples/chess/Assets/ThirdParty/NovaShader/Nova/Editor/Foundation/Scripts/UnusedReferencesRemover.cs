...
namespace Nova.Editor.Foundation.Scripts
{
    public static class UnusedReferencesRemover
    {
        ...
        {
            var mode = (ColorCorrectionMode)
                material.GetFloat(MaterialPropertyNames.ColorCorrectionMode);
            switch (mode)
            {
        ...
        {
            var mode = (AlphaTransitionMode)
                material.GetFloat(MaterialPropertyNames.AlphaTransitionMode);
            ...
            var mapMode = (AlphaTransitionMapMode)
                material.GetFloat(MaterialPropertyNames.AlphaTransitionMapMode);
            // 1st Texture
            {
                switch (mapMode)
                {
            ...
            // 2nd Texture
            {
                var blendMode = (AlphaTransitionBlendMode)
                    material.GetFloat(MaterialPropertyNames.AlphaTransitionSecondTextureBlendMode);
        ...
        {
            var areaMode = (EmissionAreaType)
                material.GetFloat(MaterialPropertyNames.EmissionAreaType);
            ...
            // EmissionMap
            {
                ...
                {
                    var mode = (EmissionMapMode)
                        material.GetFloat(MaterialPropertyNames.EmissionMapMode);
                    switch (mode)
                    {
            ...
            // GradientMap
            {
                ...
                var colorType = (EmissionColorType)
                    material.GetFloat(MaterialPropertyNames.EmissionColorType);
        ...
        private static void FixSurfaceMap(Material material)
        {
            ...
            var workFlowMode = (LitWorkflowMode)
        ...
        private static void ClearTexture(Material material, string propertyName)
        {
            ...
            Debug.Log(
                $"[NOVA] {material.name}: Removed unused texture from property: {propertyName}"
            );
        }
