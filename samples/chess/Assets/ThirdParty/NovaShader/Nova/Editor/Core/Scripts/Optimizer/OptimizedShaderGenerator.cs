...
namespace Nova.Editor.Core.Scripts.Optimizer
{
...
#endif
    {
        ...
        public static void Generate(string outputFolderPath)
        {
            ...
            foreach (var assetPath in uberShaderPaths)
                Generate(outputFolderPath, assetPath, relativePath);
        }
        ...
        private static void Generate(
            ...
            string assetPath,
            string additionalIncludePath
        )
        {
            var source = File.ReadAllText(assetPath);

            if (!IsTargetShader(assetPath, source))
                return;
            ...
            var renderTypeNames = new[] { "Opaque", "Transparent", "Cutout" };
            ...
            {
                var postFixName =
                    $"(Optimized {renderTypeNames[renderType]} {UsedPassNames[lightMode]})";
                if (!Directory.Exists(outputFolderPath))
                    Directory.CreateDirectory(outputFolderPath);
                var optimizedPath = Path.Combine(
                    $"{outputFolderPath}/",
                    Path.GetFileNameWithoutExtension(assetPath) + $"{postFixName}.shader"
                );
                var optimizedSource = "";
                if (source.Contains("Nova/Particles/UberUnlit"))
                    optimizedSource = source.Replace(
                ...
                optimizedSource = OptimizeShader(
                    optimizedSource,
                    requiredLightModes[lightMode],
        ...
        private static string OptimizeShader(
            string shaderCode,
            OptionalShaderPass requiredLightMode,
            RenderType renderType
        )
        {
        ...
        private static string OptimizeShaderPass(
            string shaderCode,
            string passName,
            OptionalShaderPass requiredLightMode,
            RenderType renderType
        )
        {
            ...
            while (i < lineCount)
            {
                ...
                while (i < lineCount && !lines[i].Contains("{"))
                    i++;
                ...
                {
                    Debug.LogWarning(
                        "[OptimizeShaderPasses] Failed to parse Pass block: unmatched braces."
                    );
                    break;
                }
                ...
                if (enablePass)
                {
                    ...
                    if (
                        ...
                        for (var k = startLine; k <= passEnd; k++)
                        {
                            ...
                            if (!ShaderFeaturePragmaRegex.IsMatch(lineCache))
                                continue;
                            if (!ShaderKeywordRegex.IsMatch(lineCache))
                                continue;
                            lines[k] = "";
                        }
                }
