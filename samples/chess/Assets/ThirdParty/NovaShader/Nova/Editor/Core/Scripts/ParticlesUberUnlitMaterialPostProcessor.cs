...
namespace Nova.Editor.Core.Scripts
{
    ...
    public static class ParticlesUberUnlitMaterialPostProcessor
    {
        ...
        private static readonly int VertexAlphaModeId = Shader.PropertyToID(
            MaterialPropertyNames.VertexAlphaMode
        );
        private static readonly int BaseMapModeId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapMode
        );

        private static readonly int BaseMapMirrorSamplingId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapMirrorSampling
        );

        private static readonly int BaseMapId = Shader.PropertyToID(MaterialPropertyNames.BaseMap);
        private static readonly int BaseMap2DArrayId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMap2DArray
        );
        private static readonly int BaseMap3DId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMap3D
        );
        private static readonly int BaseMapRotationId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapRotation
        );
        private static readonly int BaseMapToneModeId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapToneMode
        );

        private static readonly int BaseMapRotationCoordId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapRotationCoord
        );

        private static readonly int BaseMapRandomRowSelectionEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapRandomRowSelectionEnabled
        );

        private static readonly int BaseMapChannelId = Shader.PropertyToID(
            MaterialPropertyNames.BaseMapChannel
        );

        // Random Row Selection Property IDs
        private static readonly int TintMapRandomRowSelectionEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.TintMapRandomRowSelectionEnabled
        );
        private static readonly int ParallaxMapRandomRowSelectionEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.ParallaxMapRandomRowSelectionEnabled
        );
        ...
        private static readonly int AlphaTransitionMapSecondTextureRandomRowSelectionEnabledId =
            Shader.PropertyToID(
                MaterialPropertyNames.AlphaTransitionMapSecondTextureRandomRowSelectionEnabled
            );
        private static readonly int EmissionMapRandomRowSelectionEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.EmissionMapRandomRowSelectionEnabled
        );

        private static readonly int TintAreaModeId = Shader.PropertyToID(
            MaterialPropertyNames.TintAreaMode
        );
        private static readonly int TintMapModeId = Shader.PropertyToID(
            MaterialPropertyNames.TintColorMode
        );
        private static readonly int FlowMapId = Shader.PropertyToID(MaterialPropertyNames.FlowMap);
        private static readonly int FlowMapTargetId = Shader.PropertyToID(
            MaterialPropertyNames.FlowMapTarget
        );

        private static readonly int FlowIntensityCoordId = Shader.PropertyToID(
            MaterialPropertyNames.FlowIntensityCoord
        );

        private static readonly int ParallaxMapModeId = Shader.PropertyToID(
            MaterialPropertyNames.ParallaxMapMode
        );
        private static readonly int ParallaxMapId = Shader.PropertyToID(
            MaterialPropertyNames.ParallaxMap
        );

        private static readonly int ParallaxMap2DArrayId = Shader.PropertyToID(
            MaterialPropertyNames.ParallaxMap2DArray
        );

        private static readonly int ParallaxMap3DId = Shader.PropertyToID(
            MaterialPropertyNames.ParallaxMap3D
        );
        private static readonly int ParallaxMapTargetId = Shader.PropertyToID(
            MaterialPropertyNames.ParallaxMapTarget
        );

        private static readonly int ColorCorrectionModeId = Shader.PropertyToID(
            MaterialPropertyNames.ColorCorrectionMode
        );

        private static readonly int AlphaTransitionMapId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMap
        );

        private static readonly int AlphaTransitionMap2DArrayId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMap2DArray
        );

        private static readonly int AlphaTransitionMap3DId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMap3D
        );

        private static readonly int AlphaTransitionMapSecondTextureId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMapSecondTexture
        );

        private static readonly int AlphaTransitionMapSecondTexture2DArrayId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMapSecondTexture2DArray
        );

        private static readonly int AlphaTransitionMapSecondTexture3DId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMapSecondTexture3D
        );

        private static readonly int AlphaTransitionProgressCoordId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionProgressCoord
        );

        private static readonly int AlphaTransitionModeId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMode
        );

        private static readonly int AlphaTransitionMapModeId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMapMode
        );

        private static readonly int AlphaTransitionSecondTextureBlendMode = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionSecondTextureBlendMode
        );

        private static readonly int AlphaTransitionMapSecondTextureMapModeId = Shader.PropertyToID(
            MaterialPropertyNames.AlphaTransitionMapSecondTextureMapMode
        );

        private static readonly int EmissionAreaTypeId = Shader.PropertyToID(
            MaterialPropertyNames.EmissionAreaType
        );
        private static readonly int EmissionColorTypeId = Shader.PropertyToID(
            MaterialPropertyNames.EmissionColorType
        );
        private static readonly int EmissionMapModeId = Shader.PropertyToID(
            MaterialPropertyNames.EmissionMapMode
        );

        private static readonly int RimTransparencyEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.RimTransparencyEnabled
        );

        private static readonly int LuminanceTransparencyEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.LuminanceTransparencyEnabled
        );

        private static readonly int SoftParticlesEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.SoftParticlesEnabled
        );

        private static readonly int VertexDeformationMapId = Shader.PropertyToID(
            MaterialPropertyNames.VertexDeformationMap
        );

        private static readonly int DepthFadeEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.DepthFadeEnabled
        );
        private static readonly int RenderTypeId = Shader.PropertyToID(
            MaterialPropertyNames.RenderType
        );
        private static readonly int QueueOffsetId = Shader.PropertyToID(
            MaterialPropertyNames.QueueOffset
        );
        private static readonly int BlendSrcId = Shader.PropertyToID(
            MaterialPropertyNames.BlendSrc
        );
        private static readonly int BlendDstId = Shader.PropertyToID(
            MaterialPropertyNames.BlendDst
        );
        private static readonly int ZWriteId = Shader.PropertyToID(MaterialPropertyNames.ZWrite);
        private static readonly int ZWriteOverrideId = Shader.PropertyToID(
            MaterialPropertyNames.ZWriteOverride
        );

        private static readonly int TransparentBlendModeId = Shader.PropertyToID(
            MaterialPropertyNames.TransparentBlendMode
        );

        private static readonly int ShadowCasterEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.ShadowCasterEnabled
        );

        private static readonly int ShadowCasterAlphaTestEnabledId = Shader.PropertyToID(
            MaterialPropertyNames.ShadowCasterAlphaTestEnabled
        );
        ...
        private static void SetupDrawSettingsMaterialKeywords(Material material)
        {
            ...
            var vertexAlphaAsTransitionProgressEnabled =
                vertexAlphaMode == VertexAlphaMode.TransitionProgress;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.VertexAlphaAsTransitionProgress,
        ...
        private static void SetupBaseColorMaterialKeywords(Material material)
        {
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseSamplerStatePointMirror,
                false
            );
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseSamplerStateLinearMirror,
                false
            );
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseSamplerStateTrilinearMirror,
                false
            );
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseMapToneModeTriTone,
                toneMode == BaseMapToneMode.Tritone
            );
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseMapToneModePentone,
                toneMode == BaseMapToneMode.Pentone
            );
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseMapRandomRowSelectionEnabled,
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.BaseMapChannelEnabled,
                baseMapChannelEnabled
            );
            ...
            var tintMapRandomRowSelectionEnabled =
                (
                    tintColorMode == TintColorMode.FlipBook
                    || tintColorMode == TintColorMode.FlipBookBlending
                )
                && material.GetFloat(TintMapRandomRowSelectionEnabledId) > 0.5f;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.TintMapRandomRowSelectionEnabled,
                tintMapRandomRowSelectionEnabled
            );

            var colorCorrectionMode = (ColorCorrectionMode)material.GetFloat(ColorCorrectionModeId);
        ...
        private static void SetupFlowMapMaterialKeywords(Material material)
        {
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.FlowMapTargetBase,
                baseEnabled
            );
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.FlowMapTargetTint,
                tintEnabled
            );

            var alphaTransitionEnabled =
                hasFlowMap && (flowMapTarget & FlowMapTarget.AlphaTransitionMap) != 0;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.FlowMapTargetAlphaTransition,
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.FlowMapTargetEmission,
                emissionEnabled
            );
        }
        ...
        private static void SetupParallaxMapMaterialKeywords(Material material)
        {
            ...
            var baseEnabled =
                hasParallaxMap && (parallaxMapTarget & ParallaxMapTarget.BaseMap) != 0;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.ParallaxMapTargetBase,
                baseEnabled
            );

            var tintEnabled =
                hasParallaxMap && (parallaxMapTarget & ParallaxMapTarget.TintMap) != 0;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.ParallaxMapTargetTint,
                tintEnabled
            );

            var emissionEnabled =
                hasParallaxMap && (parallaxMapTarget & ParallaxMapTarget.EmissionMap) != 0;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.ParallaxMapTargetEmission,
                emissionEnabled
            );
            ...
            var parallaxMapRandomRowSelectionEnabled =
                (
                    parallaxMapMode == ParallaxMapMode.FlipBook
                    || parallaxMapMode == ParallaxMapMode.FlipBookBlending
                )
                && material.GetFloat(ParallaxMapRandomRowSelectionEnabledId) > 0.5f;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.ParallaxMapRandomRowSelectionEnabled,
        ...
        {
            var alphaTransitionEnabled =
                material.GetTexture(AlphaTransitionMapId) != null
            ...
            var fadeTransitionEnabled =
                alphaTransitionEnabled && alphaTransitionMode == AlphaTransitionMode.Fade;
            var dissolveTransitionEnabled =
                alphaTransitionEnabled && alphaTransitionMode == AlphaTransitionMode.Dissolve;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.FadeTransitionEnabled,
                fadeTransitionEnabled
            );
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.DissolveTransitionEnabled,
                dissolveTransitionEnabled
            );

            var alphaTransitionMapMode = (AlphaTransitionMapMode)
                material.GetFloat(AlphaTransitionMapModeId);
            ...
            var alphaTransitionMapRandomRowSelectionEnabled =
                (
                    alphaTransitionMapMode == AlphaTransitionMapMode.FlipBook
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.AlphaTransitionMapRandomRowSelectionEnabled,
                alphaTransitionMapRandomRowSelectionEnabled
            );

            // 2nd Texture
            {
                var blendMode = (AlphaTransitionBlendMode)
                    material.GetFloat(AlphaTransitionSecondTextureBlendMode);
                var secondTexEnabled =
                    material.GetTexture(AlphaTransitionMapSecondTextureId) != null
                ...
                var secondTexActive =
                    secondTexEnabled && blendMode != AlphaTransitionBlendMode.None;

                MaterialEditorUtility.SetKeyword(
                    material,
                    ShaderKeywords.AlphaTransitionBlendSecondTexAverage,
                    secondTexEnabled && blendMode == AlphaTransitionBlendMode.Average
                );
                MaterialEditorUtility.SetKeyword(
                    material,
                    ShaderKeywords.AlphaTransitionBlendSecondTexMultiply,
                ...
                var secondTextureMapMode = (AlphaTransitionMapMode)
                    material.GetFloat(AlphaTransitionMapSecondTextureMapModeId);
                if (secondTextureMapMode == AlphaTransitionMapMode.SingleTexture)
                {
                    ...
                    )
                    {
                        ...
                        material.SetFloat(
                            AlphaTransitionMapSecondTextureMapModeId,
                            (float)AlphaTransitionMapMode.FlipBook
                        );
                    }
                    else if (
                        alphaTransitionMapMode == AlphaTransitionMapMode.FlipBookBlending
                    ...
                    )
                    {
                        ...
                        material.SetFloat(
                            AlphaTransitionMapSecondTextureMapModeId,
                            (float)AlphaTransitionMapMode.FlipBookBlending
                        );
                    }
                }

                foreach (
                    AlphaTransitionMapMode value in Enum.GetValues(typeof(AlphaTransitionMapMode))
                )
                {
                ...
                var alphaTransitionMapSecondTextureRandomRowSelectionEnabled =
                    ...
                    && material.GetFloat(AlphaTransitionMapSecondTextureRandomRowSelectionEnabledId)
                        > 0.5f;
                MaterialEditorUtility.SetKeyword(
                    material,
                    ShaderKeywords.AlphaTransitionMapSecondTextureRandomRowSelectionEnabled,
        ...
        private static void SetupTransparencyMaterialKeywords(Material material)
        {
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.TransparencyByRim,
                rimTransparencyEnabled
            );
            var luminanceTransparencyEnabled =
                material.GetFloat(LuminanceTransparencyEnabledId) > 0.5f;
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.TransparencyByLuminance,
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.SoftParticlesEnabled,
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.DepthFadeEnabled,
                depthFadeEnabled
            );
        }
        ...
        private static void SetupVertexDeformationMaterialKeywords(Material material)
        {
            ...
            MaterialEditorUtility.SetKeyword(
                material,
                ShaderKeywords.VertexDeformationEnabled,
        ...
        private static void SetupShadowCasterMaterialKeywords(Material material)
        {
            ...
            if (shadowCasterEnabled)
            {
                ...
                MaterialEditorUtility.SetKeyword(
                    material,
                    ShaderKeywords.ShadowCasterAlphaTestEnable,
                    alphaTestEnabled
                );
            }
        }
        ...
        public static void SetupMaterialBlendMode(Material material)
        {
            ...
            int GetZWriteValue(int defaultValue)
            {
                ...
                var zWrite =
                    zWriteOverride == ZWriteOverride.Auto ? defaultValue : (int)zWriteOverride;
                return zWrite;
            }
