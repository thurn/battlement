...
namespace Nova.Editor.Core.Scripts
{
    ...
    public static class RendererErrorHandler
    {
        ...
        public static bool CheckErrorWithMaterial(Material material)
        {
            ...
            if (allRenderersWithMaterial == null || allRenderersWithMaterial.Count == 0)
                return false;

            SetupCorrectVertexStreams(
                material,
                out var correctVertexStreams,
                out var correctVertexStreamsInstanced
            );
            return CheckError(
                allRenderersWithMaterial,
                material,
        ...
        {
            var isCustomCoordUsed =
                IsCustomCoordUsed(commonMaterialProperties.VertexDeformationMapOffsetXCoordProp)
        ...
        private static bool IsCustomCoordUsedInBaseMap(
            ParticlesUberCommonMaterialProperties commonMaterialProperties
        )
        {
            // Check basic texture coordinate usage
            if (
                IsCustomCoordUsed(commonMaterialProperties.BaseMapOffsetXCoordProp)
            ...
            var baseMapMode = (BaseMapMode)
                commonMaterialProperties.BaseMapModeProp.Value.floatValue;
            ...
            if (baseMapMode == BaseMapMode.FlipBook || baseMapMode == BaseMapMode.FlipBookBlending)
            {
                ...
                if (
                    commonMaterialProperties.BaseMapRandomRowSelectionEnabledProp.Value.floatValue
                    > 0.5f
                )
                {
                    var randomCoord = (CustomCoord)
                        commonMaterialProperties.BaseMapRandomRowCoordProp.Value.floatValue;
            ...
            var toneMode = (BaseMapToneMode)
                commonMaterialProperties.BaseMapToneModeProp.Value.floatValue;
            if (toneMode != BaseMapToneMode.None)
            {
                if (
                    IsCustomCoordUsed(commonMaterialProperties.ToneShadowsCoordProp)
                ...
                {
                    if (
                        IsCustomCoordUsed(commonMaterialProperties.ToneBrightsCoordProp)
        ...
        private static bool IsCustomCoordUsedInTintColor(
            ParticlesUberCommonMaterialProperties commonMaterialProperties
        )
        {
            var isCustomCoordUsed = false;
            var tintAreaMode = (TintAreaMode)
                commonMaterialProperties.TintAreaModeProp.Value.floatValue;
            if (tintAreaMode != TintAreaMode.None)
            {
                isCustomCoordUsed |= IsCustomCoordUsed(
                    commonMaterialProperties.TintMapBlendRateCoordProp
                );
                if (tintAreaMode == TintAreaMode.Rim)
                    isCustomCoordUsed |=
                        IsCustomCoordUsed(commonMaterialProperties.TintRimProgressCoordProp)
                        || IsCustomCoordUsed(commonMaterialProperties.TintRimSharpnessCoordProp);

                var tintMapMode = (TintColorMode)
                    commonMaterialProperties.TintColorModeProp.Value.floatValue;
                if (
                    tintMapMode == TintColorMode.SingleTexture
                    || tintMapMode == TintColorMode.FlipBookBlending
                    || tintMapMode == TintColorMode.FlipBook
                )
                {
                    isCustomCoordUsed |=
                        IsCustomCoordUsed(commonMaterialProperties.TintMapOffsetXCoordProp)
                    ...
                    if (tintMapMode == TintColorMode.FlipBookBlending)
                        isCustomCoordUsed |= IsCustomCoordUsed(
                            commonMaterialProperties.TintMap3DProgressCoordProp
                        );
                    else if (tintMapMode == TintColorMode.FlipBook)
                        isCustomCoordUsed |= IsCustomCoordUsed(
                            commonMaterialProperties.TintMapProgressCoordProp
                        );
                }
            }
        ...
        private static bool IsCustomCoordUsedInFlowMap(
            ParticlesUberCommonMaterialProperties commonMaterialProperties
        )
        {
        ...
        private static bool IsCustomCoordUsedInParallax(
            ParticlesUberCommonMaterialProperties commonMaterialProperties
        )
        {
            var isCustomCoordUsed =
                IsCustomCoordUsed(commonMaterialProperties.ParallaxMapOffsetXCoordProp)
                || IsCustomCoordUsed(commonMaterialProperties.ParallaxMapOffsetYCoordProp);
            isCustomCoordUsed |=
                (ParallaxMapMode)commonMaterialProperties.ParallaxMapModeProp.Value.floatValue
        ...
        )
        {
            ...
            var isFlipBook =
                (AlphaTransitionMapMode)props.AlphaTransitionMapModeProp.Value.floatValue
                != AlphaTransitionMapMode.SingleTexture;
            // 1st texture
            {
                isCustomCoordUsed = IsCustomCoordUsed(props.AlphaTransitionProgressCoordProp);
                isCustomCoordUsed |=
                    IsCustomCoordUsed(props.AlphaTransitionMapOffsetXCoordProp)
                    || IsCustomCoordUsed(props.AlphaTransitionMapOffsetYCoordProp)
                    || IsCustomCoordUsed(props.AlphaTransitionMapRotationCoordProp);
                isCustomCoordUsed |=
                    isFlipBook && IsCustomCoordUsed(props.AlphaTransitionMapProgressCoordProp);
            }
            // 2nd texture
            {
                if (
                    (AlphaTransitionBlendMode)
                        props.AlphaTransitionSecondTextureBlendModeProp.Value.floatValue
                    != AlphaTransitionBlendMode.None
                )
                {
                    var isSecondTextureFlipBook =
                        (AlphaTransitionMapMode)
                            props.AlphaTransitionMapSecondTextureMapModeProp.Value.floatValue
                        != AlphaTransitionMapMode.SingleTexture;
                    isCustomCoordUsed |= IsCustomCoordUsed(
                        props.AlphaTransitionProgressCoordSecondTextureProp
                    );
                    isCustomCoordUsed |=
                        IsCustomCoordUsed(props.AlphaTransitionMapSecondTextureOffsetXCoordProp)
                        || IsCustomCoordUsed(props.AlphaTransitionMapSecondTextureOffsetYCoordProp)
                        || IsCustomCoordUsed(
                            props.AlphaTransitionMapSecondTextureRotationCoordProp
                        );
                    isCustomCoordUsed |=
                        isSecondTextureFlipBook
                        && IsCustomCoordUsed(
                            props.AlphaTransitionMapSecondTextureProgressCoordProp
                        );
                }
            }
            return isCustomCoordUsed;
        ...
        private static bool IsCustomCoordUsedInEmission(
            ParticlesUberCommonMaterialProperties commonMaterialProperties
        )
        {
            var mode = (EmissionAreaType)
                commonMaterialProperties.EmissionAreaTypeProp.Value.floatValue;
            if (mode == EmissionAreaType.None)
                return false;
            var isCustomCoordUsed = IsCustomCoordUsed(
                commonMaterialProperties.EmissionIntensityCoordProp
            );
            if (mode == EmissionAreaType.ByTexture)
            {
                isCustomCoordUsed |=
                    IsCustomCoordUsed(commonMaterialProperties.EmissionMapOffsetXCoordProp)
                ...
                isCustomCoordUsed |=
                    (EmissionMapMode)commonMaterialProperties.EmissionMapModeProp.Value.floatValue
        ...
        )
        {
            var isCustomCoordUsed = false;
            var enabledRim =
                commonMaterialProperties.RimTransparencyEnabledProp.Value.floatValue > 0.5f;
            if (enabledRim)
            {
                isCustomCoordUsed |= IsCustomCoordUsed(
                    commonMaterialProperties.RimTransparencyProgressCoordProp
                );
                isCustomCoordUsed |= IsCustomCoordUsed(
                    commonMaterialProperties.RimTransparencySharpnessCoordProp
                );
            }

            var enabledLuminance =
                commonMaterialProperties.LuminanceTransparencyEnabledProp.Value.floatValue > 0.5f;
            if (enabledLuminance)
            {
                isCustomCoordUsed |= IsCustomCoordUsed(
                    commonMaterialProperties.LuminanceTransparencyProgressCoordProp
                );
                isCustomCoordUsed |= IsCustomCoordUsed(
                    commonMaterialProperties.LuminanceTransparencySharpnessCoordProp
                );
            }
        ...
        private static bool IsCustomCoordUsed(
            ParticlesUberCommonMaterialProperties commonMaterialProperties
        )
        {
            if (commonMaterialProperties == null)
                return false;
        ...
        internal static void SetupCorrectVertexStreams(
            Material material,
        ...
        )
        {
            ...
            ParticlesUberCommonMaterialProperties commonMaterialProperties = new(
                materialProperties
            );
            ...
            {
                var useParallaxMap =
                    material.IsKeywordEnabled(ShaderKeywords.ParallaxMapTargetBase)
        ...
        {
            return particleSystem.enableGPUInstancing
                && particleSystem.renderMode == ParticleSystemRenderMode.Mesh;
        }

        private static bool CompareVertexStreams(
            List<ParticleSystemVertexStream> a,
...
#else
            var renderers =
                Object.FindObjectsOfType(typeof(ParticleSystemRenderer))
...
#endif
            if (renderers == null)
                return new List<ParticleSystemRenderer>();
        ...
        )
        {
            ...
            foreach (var renderer in renderers)
            {
                ...
                var streamsValid = CompareVertexStreams(
                    rendererStreams,
                    IsEnabledGPUInstancing(renderer)
                        ? correctVertexStreamsInstanced
                        : correctVertexStreams
                );
                if (streamsValid)
        ...
        )
        {
            foreach (var renderer in renderers)
            {
                ...
                if (renderer.trailMaterial == targetMaterial)
                    renderer.SetActiveTrailVertexStreams(
                        IsEnabledGPUInstancing(renderer)
                            ? correctVertexStreamsInstanced
                            : correctVertexStreams
                    );
