...
namespace Nova.Editor.Core.Scripts
{
    internal static class MaterialEditorUtility
    {
        ...
        private static readonly Color HeaderBackgroundColorDark = new(
            40.0f / 256.0f,
            40.0f / 256.0f,
            40.0f / 256.0f
        );

        private static readonly Color ContentsBackgroundColorDark = new(
            48.0f / 256.0f,
            48.0f / 256.0f,
            48.0f / 256.0f
        );

        private static readonly Color BorderColorDark = new(
            16.0f / 256.0f,
            16.0f / 256.0f,
            16.0f / 256.0f
        );

        private static readonly Color HeaderBackgroundColorLight = new(
            170.0f / 256.0f,
            170.0f / 256.0f,
            170.0f / 256.0f
        );

        private static readonly Color ContentsBackgroundColorLight = new(
            193.0f / 256.0f,
            193.0f / 256.0f,
            193.0f / 256.0f
        );

        private static readonly Color BorderColorLight = new(
            101.0f / 256.0f,
            101.0f / 256.0f,
            101.0f / 256.0f
        );
        ...
        private static Color BorderColor =>
            EditorGUIUtility.isProSkin ? BorderColorDark : BorderColorLight;
        ...
        private static Texture2D HeaderBackgroundTexture
        {
            get
            {
                ...
                {
                    _headerBackgroundTexture = new Texture2D(
        ...
        private static Texture2D ContentsBackgroundTexture
        {
            get
            {
                ...
                {
                    _contentsBackgroundTexture = new Texture2D(
                        ProceduralTextureSize,
                        ProceduralTextureSize
                    );
                    for (var i = 0; i < ProceduralTextureSize; i++)
        ...
        private static GUIStyle HeaderStyle
        {
            get
            {
                if (_headerStyle == null || _headerBackgroundTexture == null)
                    ...
                    {
                        normal = new GUIStyleState { background = HeaderBackgroundTexture },
        ...
        private static GUIStyle ContentsStyle
        {
            get
            {
                if (_contentsStyle == null || _contentsBackgroundTexture == null)
                    ...
                    {
                        normal = new GUIStyleState { background = ContentsBackgroundTexture },
                        border = new RectOffset(1, 1, 1, 1),
        ...
        public static void DrawSmallTexture(
            MaterialEditor editor,
            string label,
            MaterialProperty textureProp,
            MaterialProperty channelsXProperty,
            MaterialProperty multipliedValueProp
        )
        {
            // Texture
            Type textureType;
            ...
            if (multipliedValueProp == null)
                ...
                offsetYFromTextureRectTop =
                    textureRect.height / 2 - EditorGUIUtility.singleLineHeight / 2;
            ...
            if (channelsXProperty != null)
            {
                ...
                var xPropertyXOffset =
                    EditorGUIUtility.labelWidth - mapWidth - offsetXFromTextureRectLeft;
        ...
        public static void DrawTexture<TCustomCoord>(
            MaterialEditor editor,
        ...
        public static void DrawTexture<TCustomCoord>(
            MaterialEditor editor,
            MaterialProperty textureProperty,
            MaterialProperty offsetCoordXProperty,
            MaterialProperty offsetCoordYProperty,
            MaterialProperty channelsXProperty,
            MaterialProperty channelsYProperty
        )
            where TCustomCoord : Enum
        {
            DrawTexture<TCustomCoord>(
                ...
                textureProperty,
                true,
                offsetCoordXProperty,
                offsetCoordYProperty,
                channelsXProperty,
                channelsYProperty
            );
        }

        private static void DrawTexture<TCustomCoord>(
            MaterialEditor editor,
            MaterialProperty textureProperty,
            bool drawTilingAndOffset,
            MaterialProperty offsetCoordXProperty,
            ...
            where TCustomCoord : Enum
        {
            ...
            if (useChannels)
                propertyCount += 1;
            var contentsHeight =
                propertyCount * EditorGUIUtility.singleLineHeight
            ...
            using (new ResetIndentLevelScope())
            {
                ...
                if (drawTilingAndOffset)
                    using (var changeCheckScope = new EditorGUI.ChangeCheckScope())
                    {
                        ...
                        if (changeCheckScope.changed)
                        {
                            textureScaleAndOffset = new Vector4(
                                tiling.x,
                                tiling.y,
                                offset.x,
                                offset.y
                ...
                if (useOffsetCoord)
                {
                    ...
                    DrawEnumContentsProperty<TCustomCoord>(
                        editor,
                        xPropertyRect,
                        offsetCoordXProperty
                    );
                ...
                if (useChannels)
                {
                    ...
                    xPropertyRect.xMin += 12;
                    EditorGUI.LabelField(xRect, new GUIContent("X"));
                    DrawEnumContentsProperty<ColorChannels>(
                        editor,
                        xPropertyRect,
                        channelsXProperty
                    );
        ...
        public static void DrawBaseMapChannel(
            MaterialEditor editor,
            MaterialProperty baseMapChannelProperty
        )
        {
            DrawProperty(
                "Channel",
        ...
        public static void DrawPropertyAndCustomCoord<T>(
            MaterialEditor editor,
            string label,
        ...
        public static void DrawIntRangeProperty(
            MaterialEditor editor,
            string label,
            MaterialProperty property,
            int min,
        ...
        /// <param name="min"></param>
        /// <param name="max"></param>
        public static void DrawFloatRangeProperty(
            MaterialEditor editor,
            string label,
            MaterialProperty property,
            float min,
        ...
        public static void DrawEnumProperty<T>(
            MaterialEditor editor,
            string label,
            MaterialProperty property
        )
            where T : Enum
        {
            DrawProperty(
                ...
                rect =>
                {
                    DrawEnumContentsProperty<T>(editor, rect, property);
                }
            );
        }
        ...
        /// <param name="property"></param>
        /// <typeparam name="T"></typeparam>
        public static void DrawEnumFlagsProperty<T>(
            MaterialEditor editor,
            string label,
            MaterialProperty property
        )
            where T : Enum
        {
            DrawProperty(
                ...
                rect =>
                {
                    DrawEnumFlagsContentsProperty<T>(editor, rect, property);
                }
            );
        }
        ...
        /// <param name="label"></param>
        /// <param name="property"></param>
        public static void DrawToggleProperty(
            MaterialEditor editor,
            string label,
            MaterialProperty property
        )
        ...
        /// <param name="label"></param>
        /// <param name="property"></param>
        public static void DrawVector2Property(
            MaterialEditor editor,
            string label,
            MaterialProperty property
        )
        ...
        public static void DrawTwoFloatProperties(
            string title,
        ...
        )
        {
            ...
            using (var ccs = new EditorGUI.ChangeCheckScope())
            {
                ...
                var prop1Rect = new Rect(
                    rect.x + labelWidth,
                    rect.y,
                    (rect.width - labelWidth) * 0.5f,
                    EditorGUIUtility.singleLineHeight
                );
                var prop1Value = EditorGUI.FloatField(prop1Rect, prop1Label, prop1.floatValue);

                var prop2Rect = new Rect(
                    prop1Rect.x + prop1Rect.width,
                    rect.y,
                    prop1Rect.width,
                    EditorGUIUtility.singleLineHeight
                );
        ...
        public static void DrawEnumContentsProperty<T>(
            MaterialEditor editor,
            Rect rect,
            MaterialProperty property
        )
        ...
        private static void DrawEnumFlagsContentsProperty<T>(
            MaterialEditor editor,
            Rect rect,
            MaterialProperty property
        )
            ...
            where TCustomCoord : Enum
        {
            ...
            if (enabledProp.floatValue < 0.5f)
                return;
        ...
        private static void DrawRandomCoordProperty<TCustomCoord>(
            MaterialEditor editor,
            MaterialProperty randomCoordProp
        )
            where TCustomCoord : Enum
        ...
        private static TCustomCoord GetValidCustomCoord<TCustomCoord>(
            MaterialProperty randomCoordProp
        )
            where TCustomCoord : Enum
        {
            var coord = (TCustomCoord)
                Enum.ToObject(typeof(TCustomCoord), Convert.ToInt32(randomCoordProp.floatValue));
        ...
        private static void ValidateRowConfiguration(
            MaterialProperty rowCountProp,
            MaterialProperty sliceCountProp
        )
        {
