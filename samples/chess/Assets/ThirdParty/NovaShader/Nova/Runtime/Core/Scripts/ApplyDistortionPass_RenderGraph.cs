...
namespace Nova.Runtime.Core.Scripts
{
    public partial class ApplyDistortionPass : ScriptableRenderPass
    {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            if (
                !_applyToSceneView
                && frameData.Get<UniversalCameraData>().cameraType == CameraType.SceneView
            )
                return;
            ...
            {
                Debug.LogError(
                    "[NOVA] Cannot execute ApplyDistortionPass. DistortedUvBufferPass must be executed."
                );
                return;
            }
                ...
                builder.SetRenderFunc(
                    static (PassData data, RasterGraphContext context) =>
                    {
                        ...
                        data.Material.SetTexture(
                            data.DistortionBufferPropertyId,
                            data.DistortedUvBuffer
                        );
                        Blitter.BlitTexture(
                            context.cmd,
                            data.MainTex,
