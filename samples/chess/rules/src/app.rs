...
fn review_application(name: &str) -> reactant::Application {
...
#[cfg(test)]
pub(super) fn application_with_think_time(think_time: Duration) -> reactant::Application {
  configured_application(
    Board::default(),
    None,
    VisualState::Title,
...
#[cfg(test)]
pub(super) fn application_with_position(fen: &str, think_time: Duration) -> reactant::Application {
  configured_application(
    fen
      .parse::<Board>()
...
#[cfg(test)]
pub(super) fn application_at_position(fen: &str, think_time: Duration) -> reactant::Application {
  let board = fen
    .parse::<Board>()
    .unwrap_or_else(|error| panic!("invalid chess position: {error}"));
  configured_application(
