    ...
    if self.active {
      Node::new(
        reactant::host::View::new()
          ...
          .child(
            reactant::host::Label::new(tx("CAPTURE", "Events and portals section heading.")).style(
              design_system::event_step(self.compact, self.trace.contains(&"CAPTURE"), 0),
            ),
          )
          .child(
            reactant::host::Label::new(ls(if self.compact { "v" } else { ">" }))
          ...
          .child(
            reactant::host::Label::new(tx("TARGET", "Events and portals section heading.")).style(
              design_system::event_step(self.compact, self.trace.contains(&"TARGET"), 1),
            ),
          )
          .child(
            reactant::host::Label::new(ls(if self.compact { "v" } else { ">" }))
          ...
          .child(
            reactant::host::Label::new(tx("BUBBLE", "Events and portals section heading.")).style(
              design_system::event_step(self.compact, self.trace.contains(&"BUBBLE"), 2),
            ),
          ),
      )
    } else {
      Node::new(
        reactant::host::Label::new(tx("READY", "Events and portals status message."))
          .name("events-status")
