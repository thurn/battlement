use battlement::{Color, Command, ParallelCommandGroup, Style, UiBox, UiButton, UiNode};
...
pub(crate) fn show() -> Vec<ParallelCommandGroup<Command>> {
  let transient = UiNode::new(
    TRANSIENT_CARD_ID,
    UiBox::new().style(Style::new().background_color(Color::rgb(0.08, 0.2, 0.24))),
  );
  vec![
    ...
    ParallelCommandGroup::new(vec![
      ...
      Command::update_visual_element(CALLBACK_BUTTON_ID, UiButton::new("Hide")),
    ]),
  ]
}
