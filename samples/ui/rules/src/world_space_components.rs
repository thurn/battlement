...
pub(crate) fn page(page_id: ObjectId) -> UiNode {
    UiNode::new(
        ...
        .child(node(Label::new("WORLD-SPACE INPUT").style(design_system::eyebrow())))
        .child(node(
            Label::new("One input route. Three panel modes.").style(world_space_styles::page_title()),
        ))
        .child(node(Label::new("The screen shell explains the contract, the monitor displays a leased target texture, and the cyan console is a real world-space UIDocument picked through Unity's process-wide panel input configuration.").style(world_space_styles::intro())))
        .child(
            node(VisualElement::new().style(world_space_styles::columns()))
                .child(contract_card())
                .child(target_card()),
        )
        .child(node(
            Label::new("SCREEN OVERLAY   •   TARGET TEXTURE   •   WORLD DOCUMENT")
