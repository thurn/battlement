...
namespace Masonry.Integration
{
    ...
    public sealed class MasonryIntegrationFixture : MonoBehaviour
    {
        ...
        public const string SceneAddress = "masonry/integration/scene";
        public const string PrefabAddress = "masonry/integration/prefab";
        public const string EffectAddress = "masonry/integration/effect";
        public const string MaterialAddress = "masonry/integration/material";
        public const string TextureAddress = "masonry/integration/texture";
        public const string AudioAddress = "masonry/integration/audio";
        public const string FontAddress = "masonry/integration/font";
        ...
        public GameObject? ClickTarget
        {
            get
            {
                MasonryIdentity? identity = FindIdentity(ClickTargetId);
        ...
        public Vector2 ClickTargetPosition()
        {
            Camera camera = FindObjectsByType<Camera>()
                .Single(candidate => candidate.GetComponent<MasonryIdentity>() != null);
            GameObject? target = ClickTarget;
            if (!target)
            {
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(28, 22, 700, 40), "Masonry Integration Fixture", style);
            style.fontSize = 16;
        ...
        {
            failure = message;
            Debug.LogError($"MASONRY_INTEGRATION_FAILED:{message}");
        }
        ...
        private string Diagnostics() =>
            ...
            + string.Join(
                ",",
                FindObjectsByType<MasonryIdentity>().Select(identity => identity.Id.ToString())
            );

        private static MasonryIdentity? FindIdentity(Guid id) =>
            FindObjectsByType<MasonryIdentity>().SingleOrDefault(identity => identity.Id == id);

        private static bool HasRenderer(Guid id)
        {
            MasonryIdentity? identity = FindIdentity(id);
