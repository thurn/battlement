...
namespace Battlement.Performance
{
    ...
    public sealed class BattlementPerformanceSmoke : MonoBehaviour
    {
        private static readonly string[] MarkerNames =
        {
            "Battlement.Frame",
            "Battlement.Poll",
            "Battlement.Serialization",
            "Battlement.Transport",
            "Battlement.Response.Parse",
        ...
        private void Finish(int exitCode, string? failure)
        {
            ...
            try
            {
                ...
                Debug.Log($"BATTLEMENT_PERFORMANCE_REPORT:{reportPath}");
            }
            catch (Exception exception)
            {
                Debug.LogError($"BATTLEMENT_PERFORMANCE_FAILED:{exception}");
                exitCode = 1;
            }
            finally
            {
        ...
        private string Report(string? failure)
        {
            ...
            {
                "Battlement performance smoke (diagnostic; compare trends, not hardware)",
        ...
        private static string ReportPath()
        {
            ...
            int option = Array.IndexOf(arguments, "--battlement-performance-report");
            if (option < 0 || option + 1 >= arguments.Length)
            {
                throw new InvalidOperationException(
                    "--battlement-performance-report PATH is required."
                );
            }
        ...
        {
            BattlementIdentity? identity = FindObjectsByType<BattlementIdentity>()
