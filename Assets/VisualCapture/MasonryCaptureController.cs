...
{
    internal sealed class MasonryCaptureController : MonoBehaviour
    {
        private static MasonryCaptureController? instance;

        private readonly HashSet<Key> pressedKeys = new();
        private MasonryCaptureScenario scenario = null!;
        private MasonryFrameRecorder? recorder;
        ...
        internal static void Attach(MasonryCaptureScenario selectedScenario)
        {
            if (instance != null)
            {
            ...
            var root = new GameObject("Masonry Capture Controller");
            DontDestroyOnLoad(root);
            instance = root.AddComponent<MasonryCaptureController>();
        ...
        {
            string? controlDirectory = CaptureArguments.Value("-masonryCaptureControl");
            ...
            if (CaptureArguments.Value("-masonryCaptureInputDriver") != "in-player")
            {
                return;
            }
            ...
            mouse = InputSystem.AddDevice<Mouse>("Masonry Capture Mouse");
            keyboard = InputSystem.AddDevice<Keyboard>("Masonry Capture Keyboard");
        }
        ...
        private MasonryFrameRecorder RequireRecorder()
        {
            if (recorder == null)
            {
                recorder = gameObject.AddComponent<MasonryFrameRecorder>();
            }
            return recorder;
        }
