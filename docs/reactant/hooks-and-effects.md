...
Shared control builders and pure semantic constructors are ordinary functions;
they consume no positional hook slots and may be created outside a component
render. Components that allocate membership or host references do so internally
when Reactant renders them.
