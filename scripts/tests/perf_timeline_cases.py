...
def verify(root: Path) -> None:
    ...
    record = perf_sources.ThreadRecord("fixture", path, "Fixture", "example.invalid", 0)
    cutoff = parse_timestamp("2026-01-01T14:30:00Z")
    session = perf_sources.parse_codex_rollout(record, cutoff)
    ...
    live_session = perf_sources.parse_codex_rollout(live_record, cutoff)
    ...
    assert perf_sources.parse_codex_rollout(stale_record, cutoff).status == "unknown"

    parent_path = root / "parent.jsonl"
    parent_path.write_text(json.dumps({"timestamp": "2025-12-31T12:00:00Z", "type": "response_item",
                                      "payload": {"type": "message", "role": "user", "content": "Fixture"}}) + "\n")
    parent = perf_sources.ThreadRecord("parent", parent_path, "Parent", "example.invalid", 0)
    ...
    path.write_text("".join(json.dumps(entry) + "\n" for entry in entries))
    interrupted = perf_sources.parse_codex_rollout(record, cutoff)
    ...
    span = perf_sources._tool_span(session, call, 0, 1, output)
    assert span.status == "failed" and span.content["input"] == call["arguments"]
    assert perf_sources._tool_span(session, call, 0, 1, {"output": "done"}).status == "unknown"
