...
def load_definition(path: Path) -> dict[str, Any]:
    """Load and verify the immutable 17-scenario, 35-checkpoint definition."""
    ...
    if len(samples) != 5 or len(scenarios) != 17 or len(checkpoints) != 35:
        raise RuntimeError("benchmark shape must be exactly five samples, 17 scenarios, and 35 screenshots")
