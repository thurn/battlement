...
///
/// Native builds schedule work on a dedicated parallel pool. On Web, Masonry's
...
fn build_pool() -> Result<(ThreadPool, bool), ThreadPoolBuildError> {
    // SAFETY: Unity links this symbol from MasonryWebThreading.jslib. init.js
    // initializes its positive thread count before Unity loads the Wasm module.
    let thread_count = unsafe { self::masonry_web_thread_count() }.max(1) as usize;
...
#[cfg(target_arch = "wasm32")]
unsafe extern "C" {
    fn masonry_web_thread_count() -> i32;
}
