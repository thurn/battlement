//! In-memory Masonry-controlled objects, scenes, and component state.
...
pub struct WorldTransform {
    ...
    pub rotation: masonry::Quaternion,
...
pub struct FakeAudio {
    pub(crate) address: masonry::AudioClipAddress,
...
impl FakeAudio {
    pub(crate) fn new(
        address: masonry::AudioClipAddress,
        volume: f64,
        pitch: f64,
        looping: bool,
    ) -> Self {
    ...
    #[must_use]
    pub fn address(&self) -> &masonry::AudioClipAddress {
        &self.address
    }
...
/// One Masonry-controlled GameObject and its current logical component state.
#[derive(Clone, Debug, PartialEq)]
pub struct FakeObject {
    id: masonry::ObjectId,
    parent_id: Option<masonry::ObjectId>,
...
impl FakeObject {
    ...
    #[must_use]
    pub fn local_transform(&self) -> masonry::LocalTransform {
    ...
    #[must_use]
    pub fn light(&self) -> Option<&masonry::LightState> {
        match &self.kind {
...
impl FakeWorld {
    ...
    pub fn children(&self, id: masonry::ObjectId) -> Option<impl Iterator<Item = &FakeObject>> {
    ...
    #[must_use]
    pub fn world_transform(&self, id: masonry::ObjectId) -> WorldTransform {
        let object = self
            .objects
            .get(&id)
    ...
    #[must_use]
    pub fn audio(&self, play_command_id: masonry::CommandId) -> Option<&FakeAudio> {
    ...
    #[must_use]
    pub fn global_keys(&self) -> &[masonry::KeyCode] {
        &self.global_keys
    }
    ...
    pub(crate) fn destroy_object(&mut self, id: masonry::ObjectId) {
    ...
    pub(crate) fn set_active(&mut self, id: masonry::ObjectId, active: bool) {
    ...
    pub(crate) fn set_local_position(&mut self, id: masonry::ObjectId, value: Vector3) {
        self.require_object_mut(id).local_transform.position = value;
    }

    pub(crate) fn set_world_position(&mut self, id: masonry::ObjectId, value: Vector3) {
    ...
    pub(crate) fn set_local_rotation(&mut self, id: masonry::ObjectId, value: masonry::Quaternion) {
        self.require_object_mut(id).local_transform.rotation = transform::normalize(value);
    }

    pub(crate) fn set_world_rotation(&mut self, id: masonry::ObjectId, value: masonry::Quaternion) {
        let world = self.world_transform(id);
        let parent = self
            .require_object(id)
            .parent_id
    ...
    pub(crate) fn set_local_scale(&mut self, id: masonry::ObjectId, value: Vector3) {
        self.require_object_mut(id).local_transform.scale = value;
    }

    pub(crate) fn object_mut(&mut self, id: masonry::ObjectId) -> &mut FakeObject {
        self.require_object_mut(id)
    }

    pub(crate) fn camera_mut(&mut self, id: masonry::ObjectId) -> &mut CameraState {
    ...
    pub(crate) fn set_camera_enabled(&mut self, id: masonry::ObjectId, enabled: bool) {
    ...
    pub(crate) fn light_mut(&mut self, id: masonry::ObjectId) -> &mut masonry::LightState {
        let object = self.require_object_mut(id);
        match &mut object.kind {
    ...
    pub(crate) fn ensure_animator_mut(&mut self, id: masonry::ObjectId) -> &mut AnimatorState {
        let initial = self
    ...
    pub(crate) fn animator_descriptor(&self, id: masonry::ObjectId) -> &assets::FakeAnimator {
        self.require_object(id)
            .animator_descriptor
            .as_ref()
    ...
    pub(crate) fn particles_mut(&mut self, id: masonry::ObjectId) -> &mut bool {
        self.require_object_mut(id)
            .particles_playing
            .as_mut()
    ...
    pub(crate) fn descendant_ids(&self, id: masonry::ObjectId) -> Vec<masonry::ObjectId> {
        self.require_object(id);
        self.descendants(id)
    }
    ...
    pub(crate) fn set_input_camera(&mut self, id: masonry::ObjectId) {
        let object = self.require_object(id);
        assert!(
    ...
    pub(crate) fn set_pointer_events(
        &mut self,
        id: masonry::ObjectId,
        events: Vec<masonry::PointerEvent>,
    ) {
        let object = self.require_object_mut(id);
    ...
    pub(crate) fn set_global_keys(&mut self, keys: Vec<masonry::KeyCode>) {
        self.global_keys = keys;
    }

    pub(crate) fn audio_play(&mut self, command_id: masonry::CommandId, audio: FakeAudio) {
        assert!(
    ...
    pub(crate) fn audio_mut(&mut self, command_id: masonry::CommandId) -> &mut FakeAudio {
        self.audio
            .get_mut(&command_id)
            .unwrap_or_else(|| panic!("unknown audio command: {command_id}"))
    }

    pub(crate) fn audio_remove(&mut self, command_id: masonry::CommandId) {
        assert!(
    ...
    pub(crate) fn require_object(&self, id: masonry::ObjectId) -> &FakeObject {
        self.object(id)
            .unwrap_or_else(|| panic!("unknown object: {id}"))
    }

    fn require_object_mut(&mut self, id: masonry::ObjectId) -> &mut FakeObject {
        self.objects
            .get_mut(&id)
            .unwrap_or_else(|| panic!("unknown object: {id}"))
    }

    fn remove_object(&mut self, id: masonry::ObjectId) {
        let object = self
            .objects
            .remove(&id)
            .unwrap_or_else(|| panic!("unknown object: {id}"));
    ...
    fn descendants(&self, id: masonry::ObjectId) -> Vec<masonry::ObjectId> {
    ...
    fn is_descendant(&self, candidate: masonry::ObjectId, ancestor: masonry::ObjectId) -> bool {
    ...
    fn require_same_placement(&self, scene_id: Option<SceneId>, parent_id: masonry::ObjectId) {
        assert!(
    ...
    fn active_for(&self, id: masonry::ObjectId) -> bool {
        let object = self.require_object(id);
        object.active_self
            && object
                .parent_id
