...
const ABI_V1_SYMBOL: &str = "masonry_abi_v1";
const REQUIRED_SYMBOLS: [&str; 6] = [
    "masonry_buffer_free",
    "masonry_connect",
    "masonry_engine_create",
    "masonry_engine_destroy",
    "masonry_poll",
    "masonry_submit",
];
...
fn displaced_plugin(app: &Path) -> PathBuf {
    let mut directory: OsString = app.as_os_str().to_owned();
    directory.push(format!(".masonry-replaced.{}", std::process::id()));
    PathBuf::from(directory).join(PLUGIN_NAME)
}
...
mod tests {
    ...
    fn symbol_validation_reports_every_missing_export() {
        let error = validate_symbols(&["masonry_connect".to_owned()], true).unwrap_err();
        let message = error.to_string();
        assert!(message.contains("masonry_submit"));
        assert!(!message.contains("masonry_connect,"));
    }
    ...
    fn replacement_requires_the_versioned_abi_marker() {
        ...
        assert_eq!(
            ...
            "plugin is missing required export: masonry_abi_v1"
        );
    }
    ...
    fn backup_is_stored_beside_the_application_bundle() {
        assert_eq!(
            ...
            Path::new("/build/Game.app.masonry-backup/libmasonry_rules.dylib")
        );
    }
