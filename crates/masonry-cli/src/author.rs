...
const BOOTSTRAP_IDENTIFIER: &str = "Masonry.MessagePack::Masonry.MasonryBootstrap";
...
pub(crate) fn run(
    ...
    let destination = project.join("Assets/Plugins/macOS/libmasonry_rules.dylib");
    ...
    let mut child = Command::new(tools::unity_editor(&project)?)
        ...
        .args([
            "-executeMethod",
            "Masonry.Editor.MasonryAuthoring.OpenAndPlay",
            "-logFile",
            "-",
        ])
        .env("MASONRY_AUTHOR_SCENE_PATH", &scene)
        .spawn()
...
fn detect_bootstrap_scene(project: &Path) -> Result<PathBuf> {
    ...
    match matches.as_slice() {
        ...
        [] => bail!("no scene with a MasonryBootstrap was found; pass --scene Assets/path.unity"),
        _ => bail!(
            "multiple scenes with MasonryBootstrap were found; pass --scene to select one: {}",
            matches
                .iter()
