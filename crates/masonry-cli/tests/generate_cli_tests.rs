...
impl Fixture {
    fn new(entries: Vec<Value>) -> Self {
        ...
        fs::write(
            &editor,
            "#!/bin/sh\ncp \"$MASONRY_FAKE_EXPORT\" \"$MASONRY_ADDRESS_EXPORT_PATH\"\n",
        )
        .unwrap();
    ...
    fn generate(&self, extra: &[&str]) -> Output {
        Command::new(env!("CARGO_BIN_EXE_cargo-masonry"))
            .arg("generate")
            .arg(&self.project)
            .args(extra)
            .env("UNITY_EDITOR", &self.editor)
            .env("MASONRY_FAKE_EXPORT", &self.export)
            .output()
            .unwrap()
    }
