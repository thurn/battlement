...
fn concurrent_callers_publish_once_and_reuse_complete_entry() {
  ...
  for index in 0..6 {
    ...
    threads.push(thread::spawn(move || {
      barrier.wait();
      match cache.acquire("suite-a", &identity, 10 + index).unwrap() {
...
fn interrupted_and_failed_builds_never_become_reusable() {
  ...
  let interrupted = expect_pending(cache.acquire("suite-a", &current, 1).unwrap());
  fs::write(interrupted.path().join("partial-player"), b"partial").unwrap();
  drop(interrupted);
  assert!(!entry_path(temporary.path(), &current).exists());

  let failed = expect_pending(cache.acquire("suite-a", &current, 2).unwrap());
  ...
  assert!(!entry_path(temporary.path(), &current).exists());
  assert!(matches!(
    cache.acquire("suite-a", &current, 4).unwrap(),
    BuildAccess::Build(_)
  ));
...
fn publication_requires_player_manifest_and_full_log() {
  ...
  let pending = expect_pending(cache.acquire("suite-a", &identity, 1).unwrap());
  ...
  assert!(matches!(
    cache.acquire("suite-a", &identity, 2).unwrap(),
    BuildAccess::Build(_)
  ));
}
...
fn failed_current_source_never_falls_back_to_an_older_build() {
  ...
  let older_pending = expect_pending(cache.acquire("suite-a", &older, 1).unwrap());
  ...
  let current_pending = expect_pending(cache.acquire("suite-a", &current, 2).unwrap());
  fs::write(
  ...
  assert!(matches!(
    cache.acquire("suite-a", &current, 3).unwrap(),
    BuildAccess::Build(_)
  ));
  assert!(matches!(
    cache.acquire("suite-a", &older, 3).unwrap(),
...
fn lru_pressure_skips_active_entries_and_reports_oversize_builds() {
  ...
  let pending = expect_pending(generous.acquire("suite-a", &first, 1).unwrap());
  populate(&pending, &[b'a'; 256]);
  drop(pending.publish(Path::new("player.app"), 1).unwrap().build);

  let second = identity(HASH_B, "second");
  let pending = expect_pending(generous.acquire("suite-b", &second, 2).unwrap());
...
fn suite_cleanup_is_scoped_and_global_cleanup_removes_the_rest() {
  ...
  let pending = expect_pending(cache.acquire("suite-a", &suite_a, 1).unwrap());
  populate(&pending, b"suite a");
  drop(pending.publish(Path::new("player.app"), 1).unwrap().build);
  let suite_b = identity(HASH_B, "suite-b");
  let pending = expect_pending(cache.acquire("suite-b", &suite_b, 2).unwrap());
  populate(&pending, b"suite b");
  drop(pending.publish(Path::new("player.app"), 2).unwrap().build);

  let report = cache
    .cleanup(&CleanupScope::Suite("suite-a".to_owned()), 3)
    .unwrap();
  assert_eq!(report.evicted.len(), 1);
