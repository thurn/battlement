  ...
  F: FnOnce(&Path, &Path) -> Result<()>,
{
  ...
  prepare(&project, &manifest)?;
  let package = crate::developer_tools::rules_package(&manifest)?;
  ...
  let mut child = Command::new(crate::developer_tools::unity_editor(&project)?)
    .args(["-projectPath"])
    .arg(&project)
    .args([
      "-executeMethod",
  ...
  let status =
    wait_for_child(&mut child, interrupted).context("failed to wait for the Unity Editor")?;
  if interrupted.load(Ordering::SeqCst) {
    return Ok(());
  }
  if !status.success() {
...
fn collect_bootstrap_scenes(
  ...
  for entry in
    fs::read_dir(directory).with_context(|| format!("failed to inspect {}", directory.display()))?
  {
    let path = entry?.path();
    if path.is_dir() {
