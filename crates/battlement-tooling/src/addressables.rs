...
const MARKER: &str = "// @generated by `rt addressables generate`; do not edit.\n";
...
pub fn run(project: Option<&Path>, output: Option<&Path>, check: bool) -> Result<()> {
  ...
  if check {
    if !self::same_file(&output, staging.path())? {
      bail!(
        "generated Addressables module is stale at {}; run `rt addressables generate --project {}`",
        output.display(),
        project.display()
      );
    }
...
fn project(explicit: Option<&Path>) -> Result<PathBuf> {
  ...
  bail!(
    "could not find a Unity project from {}; pass it with `rt addressables generate --project`",
    current.display()
  )
}
...
fn render(module: &Module, entries: &[ExportEntry]) -> String {
  ...
  source.push_str("/// Every generated address that maps directly to a prepared runtime asset.\n");
