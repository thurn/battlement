...
use battlement_ui::{
    ...
    TimeValue, TransformOrigin, TransitionList, TransitionProperty, Translate, UiDocument,
    ...
    world
        .replace(vec![
            UiDocument::new(ObjectId::new_v4())
                .child(UiNode::new(target_id, Box::new().style(initial))),
        ])
        .unwrap();
    ...
    world
        .update(VisualElementUpdate::Properties {
            ...
            element: UiElement::from(
                Box::default().style(Style::new().scale(Scale::uniform(1.25))),
            )
            .into(),
        })
        .unwrap();
