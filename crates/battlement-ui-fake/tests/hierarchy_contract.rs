    ...
    world
        .replace(vec![
            UiDocument::with_root_id(first_document, first_root).child(
                UiNode::new(parent, VisualElement::new())
                    .child(UiNode::new(child, VisualElement::new())),
            ),
    ...
    world
        .replace(vec![
            UiDocument::with_root_id(document_id, root_id).child(
                UiNode::new(parent_id, Box::new())
                    .child(UiNode::new(duplicate_id, Label::new("existing"))),
            ),
        ])
        .unwrap();

    let invalid = UiNode::new(detached_id, Box::new())
        .child(UiNode::new(duplicate_id, Label::new("duplicate")));
    assert_eq!(
