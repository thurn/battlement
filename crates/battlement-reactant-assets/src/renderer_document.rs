...
const asset=document.getElementById('asset');const paint=document.getElementById('paint');asset.style.left=subject.x+'px';asset.style.top=subject.y+'px';asset.style.width=subject.width+'px';asset.style.height=subject.height+'px';
if(input.fontMarker){{const face=new FontFace('BattlementReactantGenerated','url("'+markers[input.fontMarker]+'")');await face.load();document.fonts.add(face);paint.style.fontFamily='BattlementReactantGenerated';}}
for(const declaration of input.declarations){{let value=declaration.value;for(const marker in markers)value=value.split(marker).join(markers[marker]);const target=['filter','transform','transform-origin'].includes(declaration.property)?asset:paint;target.style.setProperty(declaration.property,value);if(!target.style.getPropertyValue(declaration.property))throw new Error('browser rejected '+declaration.property+': '+value);if(declaration.property==='background-clip')target.style.setProperty('-webkit-background-clip',value);}}
if(input.content!==null)paint.textContent=input.content;
await Promise.all(imageLoads);await document.fonts.ready;
if(input.fontMarker){{const size=paint.style.fontSize||'16px';if(!document.fonts.check(size+' BattlementReactantGenerated',input.content))throw new Error('local font face did not load');const raster=(text,font)=>{{const canvas=document.createElement('canvas');canvas.width=input.width*input.scale;canvas.height=input.height*input.scale;const context=canvas.getContext('2d');context.scale(input.scale,input.scale);context.font=font;context.fillText(text,0,subject.height*.8);return context.getImageData(0,0,canvas.width,canvas.height).data;}};const equal=(a,b)=>{{if(a.length!==b.length)return false;for(let i=0;i<a.length;i++)if(a[i]!==b[i])return false;return true;}};const font=getComputedStyle(paint).font;const actual=raster(input.content,font);if(equal(actual,raster(input.content,size+' __BattlementReactantUnavailable__')))throw new Error('text shaping fell back to an unavailable control family');const stripped=[...input.content].filter(character=>{{const code=character.codePointAt(0);return code!==0x200c&&code!==0x200d&&!(code>=0xfe00&&code<=0xfe0f)&&!(code>=0xe0100&&code<=0xe01ef);}}).join('');if(stripped!==input.content&&equal(actual,raster(stripped,font)))throw new Error('text shaping ignored a variation selector or joiner');const normalized=input.content.normalize('NFC');if(/\p{{Mark}}/u.test(input.content)&&normalized!==input.content&&!equal(actual,raster(normalized,font)))throw new Error('text shaping did not preserve a combining sequence');}}
