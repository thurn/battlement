...
impl Component for MovingFixture {
  fn render(&self) -> impl Render {
    ...
    let target = Node::new(
      reactant_core::host::ButtonHost::new(ls("target"))
        .name("target")
        .key(self.key)
        .element_ref(element_ref),
    );
    let sibling = Node::new(reactant_core::host::Label::new(ls("sibling")).key("sibling"));
...
impl Component for DuplicateFixture {
  fn render(&self) -> impl Render {
    ...
    let second = self
      .duplicate
      .then(|| reactant_core::host::ButtonHost::new(ls("second")).element_ref(element_ref.clone()));
    reactant_core::host::View::new()
      .child(reactant_core::host::ButtonHost::new(ls("first")).element_ref(element_ref))
      .child(second)
  }
}
...
impl Component for InvalidRenderFixture {
  fn render(&self) -> impl Render {
    ...
    reactant_core::host::ButtonHost::new(ls("target")).element_ref(element_ref)
  }
}
...
impl Component for InvalidTargetFixture {
  fn render(&self) -> impl Render {
    let element_ref = use_element_ref();
    self.handle.replace(Some(element_ref.clone()));
    reactant_core::host::Label::new(ls("not focusable")).element_ref(element_ref)
  }
}
...
impl Component for ActionFixture {
  fn render(&self) -> impl Render {
    ...
    reactant_core::host::View::new()
      .child(reactant_core::host::ButtonHost::new(ls("focus")).element_ref(button))
      .child(
        reactant_core::host::ScrollView::new()
          .child(reactant_core::host::Label::new(ls("inside")).element_ref(child))
          .element_ref(scroll),
      )
      .child(
...
fn actions_from_another_runtime_and_invalid_targets_panic() {
  ...
  second.register_root(second_document.clone(), move |_| {
    ...
    reactant_core::host::ButtonHost::new(ls("cross"))
