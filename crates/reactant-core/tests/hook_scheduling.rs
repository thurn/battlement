...
impl Component for HookMatrix {
  fn render(&self) -> impl Render {
    ...
    (
      reactant_core::host::ButtonHost::new(ls("Invoke callback")).on_click(
        move |game: &mut Game| {
...
impl Component for FailingCallback {
  fn render(&self) -> impl Render {
    ...
    (
      reactant_core::host::ButtonHost::new(ls("Fail")).on_click(move |_game: &mut Game| callback()),
      reactant_core::host::Label::new(ls(format!("value={value}"))),
    )
  }
}
