...
impl Component for PortaledButton {
  fn render(&self) -> impl Render {
    reactant_core::host::ButtonHost::new(ls(format!(
      "{} {}",
      self.name,
...
impl Component for StatefulPortal {
  fn render(&self) -> impl Render {
    ...
    reactant_core::host::Label::new(ls(format!("state {value}")))
  }
}
...
fn internal_portals_preserve_logical_ancestry_and_global_source_order() {
  ...
  reactant.register_root(second_document.clone(), move |_: &Game| {
    ContextProvider::new()
      ...
      .child(create_portal(
        reactant_core::host::ButtonHost::new(ls("B dark-b")),
        second_target.clone(),
      ))
  });
  reactant.register_root(target_document.clone(), move |_: &Game| {
    reactant_core::host::View::new()
      .child(reactant_core::host::Label::new(ls("ordinary")))
...
fn changing_a_portal_target_or_key_remounts_its_stateful_range() {
  ...
  reactant.register_root(target_document.clone(), move |game: &Game| {
    (
      reactant_core::host::View::new().portal_target(first.clone()),
      reactant_core::host::View::new()
        .key(game.target_key)
...
fn missing_target_is_transactional() {
  ...
  reactant.register_root(source_document.clone(), move |_: &Game| {
    create_portal(
      reactant_core::host::Label::new(ls("portal")),
      source_target.clone(),
    )
  });
  reactant.register_root(target_document.clone(), move |game: &Game| {
    game
      .show_target
      .then(|| reactant_core::host::View::new().portal_target(target.clone()))
  });
  let mut game = Game {
    show_target: true,
    ..Game::default()
...
fn duplicate_target_is_rejected() {
  ...
  reactant.register_root(document, move |_: &Game| {
    (
      reactant_core::host::View::new().portal_target(target.clone()),
      reactant_core::host::View::new().portal_target(target.clone()),
    )
  });
  assert!(
...
fn cross_runtime_target_is_rejected() {
  ...
  let mut reactant = runtime_support::reactant(IdleSpawner);
  reactant.register_root(document, move |_: &Game| {
    reactant_core::host::View::new().portal_target(foreign.clone())
  });
  assert!(
    panic::catch_unwind(AssertUnwindSafe(|| {
      let _session = reactant.begin_session(&mut Game::default());
