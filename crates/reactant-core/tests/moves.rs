...
fn keyed_labels(game: &Game) -> impl Render + use<> {
  game
    .order
    .iter()
    .map(|key| reactant_core::host::Label::new(ls(format!("Label {key}"))).key(*key))
    .collect::<Vec<_>>()
}
  ...
  game
    ...
    .map(|key| {
      let children = if *key == 2 {
        ...
        vec![
          reactant_core::host::Label::new(ls(format!("{key}a"))),
          reactant_core::host::Label::new(ls(format!("{key}b"))),
        ]
      };
  ...
  reactant_core::host::ToggleButtonGroup::new()
    ...
    .child(
      game
        .order
        .iter()
        .map(|key| reactant_core::host::ButtonHost::new(ls(format!("Button {key}"))).key(*key))
        .collect::<Vec<_>>(),
    )
}
  ...
  reactant_core::host::TabView::new()
    ...
    .child(
      game
        .order
        .iter()
        .map(|key| reactant_core::host::TabHost::new(ls(format!("Tab {key}"))).key(*key))
        .collect::<Vec<_>>(),
    )
}
