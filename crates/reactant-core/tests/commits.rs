...
fn view(game: &Game) -> impl Render + use<> {
  let replaceable = if game.alternate_kind {
    Either::right(reactant_core::host::Box::new())
  } else {
    Either::left(reactant_core::host::View::new())
  };
  reactant_core::host::View::new().child((
    replaceable,
    reactant_core::host::Label::new(ls(game.left)),
    reactant_core::host::Label::new(ls(game.right)),
  ))
}
