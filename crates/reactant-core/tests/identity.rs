...
impl Component for Badge {
  fn render(&self) -> impl Render {
    reactant_core::host::Label::new(ls(format!("Badge {}", self.number)))
  }
}
...
impl Component for CountingBadge {
  fn render(&self) -> impl Render {
    ...
    reactant_core::host::Label::new(ls("counted"))
  }
}
...
fn keys_with_different_types_are_distinct() {
  ...
  reactant.register_root(document.clone(), |_| {
    (
      reactant_core::host::Label::new(ls("byte")).key(1_u8),
      reactant_core::host::Label::new(ls("word")).key(1_u16),
    )
  });
  let mut game = ();
  let initial = host_ids(render(&mut reactant, &mut game, &document));
...
fn keyed_labels(game: &KeyedGame) -> impl Render + use<> {
  game
    .order
    .iter()
    .map(|number| reactant_core::host::Label::new(ls(format!("Label {number}"))).key(*number))
    .collect::<Vec<_>>()
}
...
fn keyed_fragments(game: &KeyedGame) -> impl Render + use<> {
  game
    ...
    .map(|number| {
      Fragment::new((
        reactant_core::host::Label::new(ls(format!("{number}a"))),
        reactant_core::host::Label::new(ls(format!("{number}b"))),
      ))
      .key(*number)
    })
    .collect::<Vec<_>>()
...
fn optional_labels(game: &OptionalGame) -> impl Render + use<> {
  (
    ...
    reactant_core::host::Label::new(ls("tail")),
  )
}
...
fn mixed_labels(game: &MixedGame) -> impl Render + use<> {
  if game.keyed_first {
    vec![
      Either::left(reactant_core::host::Label::new(ls("keyed")).key(7_u8)),
      Either::right(reactant_core::host::Label::new(ls("first"))),
      Either::right(reactant_core::host::Label::new(ls("last"))),
    ]
  } else {
    vec![
      Either::right(reactant_core::host::Label::new(ls("first"))),
