...
impl Component for PairReads {
  fn render(&self) -> impl Render {
    ...
    Suspense::new(reactant_core::host::Label::new(ls("pending"))).child((
      first.then(|value| reactant_core::host::Label::new(ls(format!("first:{value}")))),
      second.then(|value| reactant_core::host::Label::new(ls(format!("second:{value}")))),
      shared.then(|value| reactant_core::host::Label::new(ls(format!("shared:{value}")))),
    ))
  }
}
...
impl Component for StatusReads {
  fn render(&self) -> impl Render {
    ...
    (
      reactant_core::host::Label::new(ls(self::status_text(use_resource(&self.ready, 1).status()))),
      reactant_core::host::Label::new(ls(self::status_text(
  ...
  fn render(&self) -> impl Render {
    Suspense::new(reactant_core::host::Label::new(ls("pending"))).child(
      use_resource(&self.resource, 1)
        .then(|value| reactant_core::host::Label::new(ls(value.to_string()))),
    )
  }
}
...
impl Component for NestedRead {
  fn render(&self) -> impl Render {
    Suspense::new(reactant_core::host::Label::new(ls("outer pending"))).child(
      Suspense::new(reactant_core::host::Label::new(ls("inner pending"))).child(
        use_resource(&self.resource, 1)
          .then(|value| reactant_core::host::Label::new(ls(value.to_string()))),
      ),
    )
...
impl Component for InvalidThen {
  fn render(&self) -> impl Render {
    use_resource(&self.resource, 1).then(|value| {
      ...
      reactant_core::host::Label::new(ls(value.to_string()))
    })
  }
}
...
impl Component for RetryRead {
  fn render(&self) -> impl Render {
    let content = Suspense::new(reactant_core::host::Label::new(ls("pending"))).child(
      use_resource(&self.resource, 1)
        .then(|value| reactant_core::host::Label::new(ls(format!("ready:{value}")))),
    );
    if self.fail {
      Err(LoadError)
    } else {
  ...
  fn render(&self) -> impl Render {
    ...
    Suspense::new(reactant_core::host::Label::new(ls("pending"))).child(
      use_resource(&self.resource, self.key).then(move |value| {
        reactant_core::host::ButtonHost::new(ls(format!("value:{value} state:{count}")))
...
impl Component for PortaledRead {
  fn render(&self) -> impl Render {
    (
      reactant_core::host::View::new().portal_target(self.target.clone()),
      Suspense::new(reactant_core::host::Label::new(ls("pending"))).child(create_portal(
        use_resource(&self.resource, 1)
          .then(|value| reactant_core::host::Label::new(ls(format!("portaled:{value}")))),
        self.target.clone(),
      )),
    )
  }
...
fn failed_reads_reach_the_nearest_error_boundary_as_the_concrete_error() {
  ...
  reactant.register_root(document.clone(), move |_| {
    ...
    ErrorBoundary::new(|_: &RenderError| reactant_core::host::Label::new(ls("outer"))).child(
      ErrorBoundary::new(move |error: &RenderError| {
        caught.set(error.downcast_ref::<LoadError>().is_some());
        reactant_core::host::Label::new(ls("failed"))
      })
      .child(FailedRead {
        resource: resource.clone(),
      }),
...
fn a_missing_boundary_panics_atomically_and_poisons_the_runtime() {
  ...
  reactant.register_root(document.clone(), move |game: &Visibility| {
    if game.load {
      ...
      Either::Right(reactant_core::host::Label::new(ls("stable")))
    }
  });
...
fn suspended_primary_survives_reconnect_and_is_released_on_unmount() {
  ...
  reactant.register_root(document.clone(), move |game: &ResourceGame| {
    if game.show {
      ...
      Either::Right(reactant_core::host::Label::new(ls("gone")))
    }
  });
  let mut world = self::begin_with(&mut reactant, &mut game, &document);
  let retained = world.element(document.root_id).unwrap().children()[0];
