...
impl Component for Fallible {
  fn render(&self) -> impl Render {
    if self.fail {
      ...
      Ok(reactant_core::host::Label::new(ls(self.label)))
    }
  }
}
...
impl Component for BoxedFailure {
  fn render(&self) -> impl Render {
    Err::<reactant_core::host::Label, _>(RenderError::from_boxed(Box::new(DomainError(
      "boxed".to_owned(),
    ))))
  }
}
...
impl Component for UpdatingFailure {
  fn render(&self) -> impl Render {
    ...
    if self.fail.get() {
      ...
      Ok(reactant_core::host::Label::new(ls(value.to_string())))
    }
  }
}
  ...
  reactant.register_root(document.clone(), |_| {
    ErrorBoundary::new(|_: &RenderError| reactant_core::host::Label::new(ls("outer"))).child((
      ErrorBoundary::new(|error: &RenderError| {
        reactant_core::host::Label::new(ls(format!(
          "inner:{}",
...
fn shared_and_erased_results_reach_boundaries_without_losing_downcasts() {
  let document = self::document();
  let shared = Rc::new(Err::<reactant_core::host::Label, _>(DomainError(
    "shared".to_owned(),
  )));
  let erased = Node::new(Err::<reactant_core::host::Label, _>(DomainError(
    "erased".to_owned(),
  )));
  let mut reactant = runtime_support::reactant(IdleSpawner);
  reactant.register_root(document.clone(), move |_| {
    (
      ErrorBoundary::new(|error: &RenderError| {
        reactant_core::host::Label::new(ls(
...
fn reset_values_and_types_retry_with_a_fresh_primary() {
  ...
  reactant.register_root(document.clone(), |game: &BoundaryGame| {
    if game.string_reset {
      Either::Left(
        ErrorBoundary::new(|_: &RenderError| reactant_core::host::Label::new(ls("fallback")))
...
fn fallback_errors_escalate_to_the_next_boundary() {
  ...
  reactant.register_root(document.clone(), |_| {
    ErrorBoundary::new(|error: &RenderError| {
      ...
      reactant_core::host::Label::new(ls("outer fallback"))
    })
    .child(
      ErrorBoundary::new(|_: &RenderError| Err::<reactant_core::host::Label, _>(FallbackError))
        .child(Fallible {
          fail: true,
          label: "primary",
        }),
...
fn errors_erased_by_host_children_reach_the_nearest_boundary() {
  ...
  reactant.register_root(document.clone(), |_| {
    ErrorBoundary::new(|error: &RenderError| {
      reactant_core::host::Label::new(ls(error.downcast_ref::<DomainError>().unwrap().to_string()))
    })
    .child(
      reactant_core::host::View::new().child(Err::<reactant_core::host::Label, _>(DomainError(
...
fn sibling_reports_run_once_in_logical_catch_order_then_render_all_roots() {
  ...
  reactant.register_root(document.clone(), move |_: &ReportGame| {
    ...
    (
      ErrorBoundary::new(|_: &RenderError| reactant_core::host::Label::new(ls("first fallback")))
        .on_error(|game: &mut ReportGame, _| game.reports.push("first"))
        .child(Fallible {
          fail: true,
          label: "first",
        }),
      ErrorBoundary::new(|_: &RenderError| reactant_core::host::Label::new(ls("second fallback")))
...
fn escaped_errors_are_atomic_and_a_corrected_root_can_retry() {
  ...
  reactant.register_root(document.clone(), |game: &RootGame| {
    if game.fail {
      ...
      Ok(reactant_core::host::Label::new(ls(game.text)))
    }
  });
...
fn boundaries_do_not_catch_panics_and_the_runtime_poisons() {
  ...
  reactant.register_root(document, |_| {
    ErrorBoundary::new(|_: &RenderError| reactant_core::host::Label::new(ls("not reached")))
      .child(Panicking)
  });
