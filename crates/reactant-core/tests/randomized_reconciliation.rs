...
impl Component for ItemView {
  fn render(&self) -> impl Render {
    ...
    let host = match self.0.kind {
      ...
      1 => Node::new(
        reactant_core::host::ButtonHost::new(ls(text))
          .name(name)
          .style(style),
      ),
      _ => Node::new(
...
impl Component for RootView {
  fn render(&self) -> impl Render {
    if self.fail {
      ...
      Ok(reactant_core::host::Label::new(ls(format!(
        "root:{}",
        self.value
      ))))
    }
...
fn duplicate_randomized_keys_fail_before_fake_world_mutation() {
  ...
  reactant.register_root(document.clone(), |duplicate: &bool| {
    vec![
      Node::new(reactant_core::host::Label::new(ls("first")).key(7_u8)),
      Node::new(
        reactant_core::host::Label::new(ls("second")).key(if *duplicate { 7_u8 } else { 8_u8 }),
      ),
    ]
  });
  let initial = reactant
...
fn render_item(item: &Item, target: &PortalTarget) -> Node {
  ...
  let boundary = ErrorBoundary::new(move |_: &RenderError| {
    reactant_core::host::Label::new(ls(format!("error:{key}")))
  })
