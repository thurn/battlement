...
fn view(game: &Game) -> impl Render + use<> {
  let child = game.show.then(|| {
    if game.alternate_kind {
      Either::right(reactant_core::host::View::new().child(label(game)))
    } else {
      Either::left(reactant_core::host::Box::new().child(label(game)))
    }
  });
  reactant_core::host::View::new().child(child)
}

fn label(game: &Game) -> reactant_core::host::Label {
...
fn hint_view(game: &HintGame) -> impl Render + use<> {
  let host = match game.hint {
    Some(hint) => reactant_core::host::View::new().usage_hints([hint]),
    None => reactant_core::host::View::new(),
  };
  host.child(reactant_core::host::Label::new(ls("child")))
}

fn conditional_part_view(game: &ConditionalPartGame) -> reactant_core::host::GroupBox {
  if game.title {
    reactant_core::host::GroupBox::new()
      .text(ls("Title"))
      .title_style(Style::new().width(20.0))
  } else {
    reactant_core::host::GroupBox::new().text(ls(""))
  }
}

fn indexed_part_view(game: &IndexedPartGame) -> reactant_core::host::RadioButtonGroup {
  if game.choices == 2 {
