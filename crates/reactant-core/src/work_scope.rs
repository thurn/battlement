...
pub(crate) fn target(command: &CommandBody) -> Option<ObjectId> {
  match command {
    ...
    CommandBody::InputSetWorldPointer(value) => Some(value.object_id),
    CommandBody::ImageSetTexture(value) => Some(value.object_id),
    CommandBody::ImageSetSize(value) => Some(value.object_id),
    CommandBody::ImageSetFit(value) => Some(value.object_id),
    CommandBody::ImageSetTint(value) => Some(value.payload.object_id),
    CommandBody::ImageSetOpacity(value) => Some(value.payload.object_id),
    CommandBody::ImageSetFaceCamera(value) => Some(value.object_id),
    CommandBody::TextSetContent(value) => Some(value.object_id),
    CommandBody::TextSetFont(value) => Some(value.object_id),
    CommandBody::TextSetSize(value) => Some(value.payload.object_id),
    CommandBody::TextSetColor(value) => Some(value.payload.object_id),
    CommandBody::TextSetAlignment(value) => Some(value.object_id),
    CommandBody::TextSetWrapping(value) => Some(value.object_id),
    CommandBody::TextSetRichText(value) => Some(value.object_id),
    CommandBody::TextSetFaceCamera(value) => Some(value.object_id),
    CommandBody::CameraSetEnabled(value) => Some(value.object_id),
    CommandBody::CameraSetPerspective(value) => Some(value.payload.object_id),
    CommandBody::CameraSetOrthographic(value) => Some(value.payload.object_id),
    CommandBody::CameraSetClipping(value) => Some(value.object_id),
    CommandBody::CameraSetClear(value) => Some(value.object_id),
    CommandBody::LightSetEnabled(value) => Some(value.object_id),
    CommandBody::LightSetType(value) => Some(value.object_id),
    CommandBody::LightSetColor(value) => Some(value.payload.object_id),
    CommandBody::LightSetIntensity(value) => Some(value.payload.object_id),
    CommandBody::LightSetRange(value) => Some(value.object_id),
    CommandBody::LightSetSpotAngle(value) => Some(value.object_id),
    CommandBody::LightSetShadows(value) => Some(value.object_id),
