...
<dict>
    ...
    <string>dev.battlement.rt-fixture</string>
</dict>
</plist>
"#;
...
fn inspect_install_verify_and_restore_keep_the_application_validly_signed() {
  ...
  rt(&["plugin", "inspect"], &[&app]);
  rt(&["plugin", "verify"], &[&plugin]);
  rt(&["plugin", "install"], &[&app, &plugin]);
  codesign(&["--verify", "--deep", "--strict"], &app);
  assert!(backup(&app).is_file());

  rt(&["plugin", "restore"], &[&app]);
  codesign(&["--verify", "--deep", "--strict"], &app);
...
fn rt(arguments: &[&str], paths: &[&Path]) {
