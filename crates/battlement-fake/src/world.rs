...
impl FakeWorld {
  ...
  pub(crate) fn replace_snapshot(
    ...
    for object in snapshot.objects {
      let fake =
        FakeObject::from_game_object(object, primary_scene_id, catalog, &next.prepared_assets);
  ...
  pub(crate) fn load_scene(
    ...
    world_validation::assert_prepared(&self.prepared_assets, PreparedAsset::Scene(address.clone()));
    assert!(
  ...
  pub(crate) fn ensure_animator_mut(&mut self, id: battlement::ObjectId) -> &mut AnimatorState {
    ...
    match &mut object.kind {
      GameObjectKind::Prefab { animator, .. } => animator.get_or_insert_with(|| AnimatorState {
        state: initial.1,
        layer: initial.0,
  ...
  fn is_descendant(&self, candidate: battlement::ObjectId, ancestor: battlement::ObjectId) -> bool {
...
impl FakeObject {
  fn from_game_object(
    ...
    let collider = collider
      && (!automatic_collider || !object.pointer_events.is_empty() || object.drag_mode.is_some());
