...
fn static_paint_updates_and_removal_preserve_the_host_and_gesture_generation() {
  ...
  runtime.register_root(document.clone(), |color: &Option<Color>| {
    let paint = color.map_or_else(PaintStyle::new, |color| {
      PaintStyle::new()
        ...
        .clip_polygon([[0., 0.], [100., 0.], [50., 100.]].map(|point| point.map(Length::percent)))
    });
    View::new()
      .paint(paint)
      .while_focus_visible(StyleTarget::new().opacity(0.8))
  });
  let mut color = Some(Color::rgba(1., 0., 0., 1.));
  ...
  let Prop::Set(first) = &host.element.visual_element().motion else {
    panic!("motion descriptor missing")
  };
  assert!(first.initial.is_none());
  assert_eq!(first.slots.len(), 1);
  assert_eq!(first.slots[0].layer, MotionLayer::FocusVisible);
  color = Some(Color::rgba(0., 1., 0., 1.));
  let (updated_host, next, motion) = self::paint_update(runtime.refresh(&mut color).unwrap());
  assert_eq!(updated_host, host.object_id);
  assert!(matches!(motion, Prop::Unset));
  assert_eq!(
    next.background_fill(),
    Some(&PaintFill::Color(color.unwrap()))
  );
  color = None;
  let (updated_host, cleared, motion) = self::paint_update(runtime.refresh(&mut color).unwrap());
  assert_eq!(updated_host, host.object_id);
  assert!(matches!(motion, Prop::Unset));
  assert_eq!(cleared, PaintStyle::new());
...
fn static_only_paint_has_no_animation_slots_or_entrance_target() {
  ...
  let (initial, commit) = runtime
    ...
    .into_parts(self::snapshot(&document));
  let _ = commit.into_groups();
  let visual = initial.ui[0].children[0].element.visual_element();
  assert!(matches!(visual.motion, Prop::Unset));
  assert!(matches!(visual.paint, Prop::Set(_)));
  let _ = runtime.shutdown(&mut ()).into_groups();
}

fn paint_update(
  commit: ReactantCommit,
) -> (ObjectId, PaintStyle, Prop<battlement::MotionDescriptor>) {
  let commands = commit
    .into_groups()
    .into_iter()
    .flatten()
  ...
  let Prop::Set(value) = &visual.paint else {
    panic!("paint update missing")
  };
  (*object_id, value.clone(), visual.motion.clone())
}
